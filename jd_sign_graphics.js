const validator = require("./JDJRValidator_Pure.js"),
    Faker = require("./sign_graphics_validate.js"),
    $ = new Env("京东签到图形验证"),
...
let message = "",
    ...
    successNum = 0,
    errorNum = 0,
    JD_API_HOST = "https://jdjoy.jd.com";
...
async function showMsg() {
    $.msg($.name, `【签到数量】:  ${turnTableId.length}个\n` + subTitle + message), $.isNode() && message && await notify.sendNotify(`${$.name}`, `【签到数量】:  ${turnTableId.length}个\n` + subTitle + message)
}
async function signRun() {
    for (let i in turnTableId) signFlag = !1, await Login(i), signFlag ? successNum++ : errorNum++, await $.wait(1e3)
}
...
function Sign(i) {
    return new Promise((resolve => {
        $.post(tasPostkUrl(turnTableId[i].id), ((err, resp, data) => {
            try {
                if (err) throw console.log(`\n${turnTableId[i].name} 签到: API查询请求失败 ‼️‼️`), new Error(err);
                data ? (data = JSON.parse(data)).success && data.data ? (data = data.data, Number(data.jdBeanQuantity) > 0 && (beanNum += Number(data.jdBeanQuantity)), signFlag = !0, console.log(`${turnTableId[i].name} 签到成功:获得 ${Number(data.jdBeanQuantity)}京豆`)) : data.errorMessage ? ((data.errorMessage.indexOf("已签到") > -1 || data.errorMessage.indexOf("今天已经签到") > -1) && (signFlag = !0), console.log(`${turnTableId[i].name} ${data.errorMessage}`)) : console.log(`${turnTableId[i].name} ${JSON.stringify(data)}`) : console.log("京豆api返回数据为空，请检查自身原因")
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function Login(i) {
    return new Promise((resolve => {
        $.get(taskUrl(turnTableId[i].id), (async (err, resp, data) => {
            try {
                if (err) console.log(`\n${turnTableId[i].name} 登录: API查询请求失败 ‼️‼️`), console.log(`${JSON.stringify(err)}`);
                else if (data)
                    if ((data = JSON.parse(data)).success && data.data)
                        if ((data = data.data).hasSign) {
                            if (data.records && data.records[0])
                                for (let i in data.records) {
                                    let item = data.records[i];
                                    if (0 == item.hasSign && 1 != item.index || i == data.records.length - 1) {
                                        0 == item.hasSign && (i -= 1), beanNum += Number(data.records[i].beanQuantity);
                                        break
                                    }
                                }
                            signFlag = !0, console.log(`${turnTableId[i].name} 已签到`)
                        } else {
                            let arr = await Faker.getBody(UA, turnTableId[i].url);
                            fp = arr.fp, await getEid(arr), await Sign(i)
                        }
                else data.errorMessage ? ((data.errorMessage.indexOf("已签到") > -1 || data.errorMessage.indexOf("今天已经签到") > -1) && (signFlag = !0), console.log(`${turnTableId[i].name} ${data.errorMessage}`)) : console.log(`${turnTableId[i].name} ${JSON.stringify(data)}`);
...
function getEid(arr) {
    return new Promise((resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                if (err) throw console.log(`\n${turnTableId[i].name} 登录: API查询请求失败 ‼️‼️`), new Error(err);
                data.indexOf("*_*") > 0 ? (data = data.split("*_*", 2), data = JSON.parse(data[1]), eid = data.eid) : console.log("京豆api返回数据为空，请检查自身原因")
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function taskUrl(turnTableId) {
    return {
        url: `${JD_API_HOST}/api/turncard/channel/detail?turnTableId=${turnTableId}&invokeKey=${$.invokeKey}`,
        headers: {
...
function tasPostkUrl(turnTableId) {
    return {
        url: `${JD_API_HOST}/api/turncard/channel/sign?turnTableId=${turnTableId}&fp=${fp}&eid=${eid}&invokeKey=${$.invokeKey}`,
        headers: {
            Accept: "application/json, text/plain, */*",
            "Accept-Encoding": "gzip, deflate, br",
...
getUA(), $.get = validator.injectToRequest($.get.bind($), "channelSign", $.UA), $.post = validator.injectToRequest($.post.bind($), "channelSign", $.UA), (async () => {
    if (cookiesArr[0]) {
        MD5();
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=(.+?);/) && cookie.match(/pt_pin=(.+?);/)[1]), $.index = i + 1, $.nickName = "", console.log(`\n开始【京东账号${$.index}】${$.nickName||$.UserName}\n`), beanNum = 0, successNum = 0, errorNum = 0, subTitle = "", lkt = (new Date).getTime(), getUA(), await signRun();
