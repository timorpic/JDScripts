...
function updateCookie(resp) {
    ...
    cookie = "";
    for (let key in newObj) key && (cookie += `${key}=${newObj[key]};`)
}
...
function genToken() {
    ...
    return new Promise((resolve => {
        $.post(config, (async (err, resp, data) => {
            updateCookie(resp);
            try {
                err ? (console.log(`${$.name} API请求失败，请检查网路重试`), console.log(`${JSON.stringify(err)}`)) : (data = JSON.parse(data), $.isvToken = data.tokenKey, cookie += `IsvToken=${data.tokenKey}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getToken2() {
    ...
    return new Promise((resolve => {
        $.post(config, (async (err, resp, data) => {
            updateCookie(resp);
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data), $.token2 = data.token)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getActCk() {
    return new Promise((resolve => {
        $.get(taskUrl("/dingzhi/qqxing/pasture/activity", "activityId=90121061401"), ((err, resp, data) => {
            updateCookie(resp);
            try {
                err && console.log(`${$.name} API请求失败，请检查网路重试`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getshopid() {
    ...
    return new Promise((resolve => {
        $.post(config, (async (err, resp, data) => {
            updateCookie(resp);
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data)).result && ($.shopid = data.data.shopId)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getMyPin() {
    ...
    return new Promise((resolve => {
        $.post(config, (async (err, resp, data) => {
            updateCookie(resp);
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data)).data && data.data.secretPin && ($.pin = data.data.secretPin, $.nickname = data.data.nickname, console.log(`欢迎回来~  ${$.nickname}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getUserInfo() {
    return new Promise((resolve => {
        let config = taskPostUrl("/wxActionCommon/getUserInfo", `pin=${encodeURIComponent($.pin)}`);
        $.post(config, (async (err, resp, data) => {
            updateCookie(resp);
            try {
                err ? console.log(`${$.name} API请求失败，请检查网路重试`) : (data = JSON.parse(data)).data ? ($.userId = data.data.id, $.pinImg = data.data.yunMidImageUrl, $.nick = data.data.nickname) : $.cando = !1
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getUid() {
    return new Promise((resolve => {
        ...
        $.post(taskPostUrl("/dingzhi/qqxing/pasture/activityContent", body), (async (err, resp, data) => {
            try {
                err ? console.log(`${$.name} API请求失败，请检查网路重试`) : (data = JSON.parse(data)).result && (3 != data.data.openCardStatus && console.log("当前未开卡,无法助力和兑换奖励哦"), console.log(`\n【京东账号${$.index}（${$.UserName}）的${$.name}好友互助码】${$.shareuuid}\n`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getinfo() {
    ...
    return new Promise((resolve => {
        $.post(config, (async (err, resp, data) => {
            ...
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                else if ((data = JSON.parse(data)).result) {
                    $.taskList = data.data.task.filter((x => 1 == x.maxNeed && 0 == x.curNum || "interact" == x.taskid)), $.taskList2 = data.data.task.filter((x => 1 != x.maxNeed && 0 == x.type)), $.draw = data.data.bags.filter((x => "drawchance" == x.bagId))[0], $.food = data.data.bags.filter((x => "food" == x.bagId))[0], $.sign = data.data.bags.filter((x => "signDay" == x.bagId))[0], $.score = data.data.score;
                    let helpinfo = data.data.task.filter((x => "share2help" == x.taskid))[0];
                    helpinfo && (console.log(`今天已有${helpinfo.curNum}人为你助力啦`), 20 == helpinfo.curNum && ($.needhelp = !1)), $.cow = `当前🐮🐮成长值：${$.score}  饲料：${$.food.totalNum-$.food.useNum}  抽奖次数：${$.draw.totalNum-$.draw.useNum}  签到天数：${$.sign.totalNum}`, $.foodNum = $.food.totalNum - $.food.useNum, console.log($.cow), $.drawchance = $.draw.totalNum - $.draw.useNum
                } else $.cando = !1, console.log(data.errorMessage)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getproduct() {
    return new Promise((resolve => {
        ...
        $.post(taskPostUrl("/dingzhi/qqxing/pasture/getproduct", body), (async (err, resp, data) => {
            updateCookie(resp);
            try {
                err ? console.log(`${$.name} API请求失败，请检查网路重试`) : (data = JSON.parse(data)).data && data.data[0] && ($.pparam = data.data[0].id, $.vid = data.data[0].venderId)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function writePersonInfo(vid) {
    return new Promise((resolve => {
        ...
        $.post(taskPostUrl("/interaction/write/writePersonInfo", body), (async (err, resp, data) => {
            updateCookie(resp);
            try {
                err ? console.log(`${$.name} API请求失败，请检查网路重试`) : (console.log("浏览：" + $.vid), console.log(data))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function exchange(id) {
    return new Promise((resolve => {
        ...
        $.post(taskPostUrl("/dingzhi/qqxing/pasture/exchange?_", body), (async (err, resp, data) => {
            updateCookie(resp);
            try {
                err ? console.log(`${$.name} API请求失败，请检查网路重试`) : (data = JSON.parse(data)).result ? (console.log(`兑换 ${data.data.rewardName}成功`), $.exchange += 20) : console.log(JSON.stringify(data))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function dotask(taskId, params) {
    ...
    return new Promise((resolve => {
        $.post(config, (async (err, resp, data) => {
            updateCookie(resp);
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data)).result ? data.data.food && console.log("操作成功,获得饲料： " + data.data.food + "  抽奖机会：" + data.data.drawChance + "  成长值：" + data.data.growUp) : console.log(data.errorMessage)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function draw() {
    ...
    return new Promise((resolve => {
        $.post(config, (async (err, resp, data) => {
            updateCookie(resp);
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data)).result ? 0 == Object.keys(data.data).length ? console.log("抽奖成功,恭喜你抽了个寂寞： ") : (console.log(`恭喜你抽中 ${data.data.prize.rewardName}`), $.drawresult += `恭喜你抽中 ${data.data.prize.rewardName} `) : console.log(data.errorMessage)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
message = "", $.shareuuid = "5afadce16d014a4694f3e50413b1e076", (async () => {
    if (cookiesArr[0]) {
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookie = cookiesArr[i], cookie) {
                if ($.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.cando = !0, $.cow = "", $.openCard = !0, $.isLogin = !0, $.needhelp = !0, $.foodNum = 0, $.nickName = "", $.drawresult = "", $.exchange = 0, console.log(`\n******开始【京东账号${$.index}】${$.nickName||$.UserName}*********\n`), !$.isLogin) {
                    $.msg($.name, "【提示】cookie已失效", `京东账号${$.index} ${$.nickName||$.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {
                        "open-url": "https://bean.m.jd.com/bean/signIndex.action"
                    }), $.isNode() && await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
                    continue
                }
                if (await genToken(), await getActCk(), await getToken2(), await getshopid(), await getMyPin(), await adlog(), await getUserInfo(), $.cando) {
                    for (await getUid($.shareuuid), await getinfo(), taskList = [...$.taskList, ...$.taskList2], j = 0; j < taskList.length; j++)
                        if (task = taskList[j], console.log(task.taskname), "interact" == task.taskid)
                            for (l = 0; l < 20 - task.curNum; l++) await dotask(task.taskid, task.params), await $.wait(500);
                        else "scansku" == task.taskid ? (await getproduct(), await writePersonInfo($.vid), await dotask(task.taskid, $.pparam)) : (await dotask(task.taskid, task.params), await $.wait(500));
                    for (await getinfo(), k = 0; k < $.drawchance; k++) await draw();
                    let exchanges = Math.floor($.foodNum / 1e3);
                    for (console.log(`可兑换 ${exchanges} 次 20京🐶`), q = 0; q < exchanges && Exchange; q++) await exchange(13);
                    await getinfo(), Exchange || console.log("你 默认 不兑换东西,请自行进去活动兑换"), message += `【京东账号${$.index}】${$.nickName||$.UserName}\n${$.cow} 兑换京🐶 ${$.exchange}  ${$.drawresult}\n`
                } else $.msg($.name, "", "跑不起来了~请自己进去一次牧场")
            } 0 != message.length && ($.isNode() ? await notify.sendNotify("星系牧场", `${message}\n牧场入口：QQ星儿童牛奶京东自营旗舰店->星系牧场`) : $.msg($.name, "", "星系牧场" + message))
