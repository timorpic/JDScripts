const $ = new Env("扫码获取京东cookie"),
    JD_UA = "Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_3_2 like Mac OS X; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8H7 Safari/6533.18.5 UCBrowser/13.4.2.1122",
...
function loginEntrance() {
    return new Promise((resolve => {
        $.get(taskUrl(), (async(err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : ($.headers = resp.headers, $.data = JSON.parse(data), await formatSetCookies($.headers, $.data))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
        ...
        $.post(taskPostUrl(), ((err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                    ...
                    $.stepsHeaders = resp.headers, data = JSON.parse(data), token = data.token;
                    ...
                    qrcode.generate(url, {
                        small: !0
                    }), console.log("请打开 京东APP 扫码登录(二维码有效期为3分钟)"), console.error(`注：如上图二维码扫描不到，请使用工具(例如在线二维码工具：https://cli.im)手动生成下面链接的二维码\n\n${url}\n\n`)
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
function checkLogin() {
    return new Promise((resolve => {
        ...
        $.post(options, ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data), $.checkLoginHeaders = resp.headers)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getCookie() {
    $.timer = setInterval((async() => {
        try {
            ...
            0 === checkRes.errcode ? ($.log("扫描登录成功\n"), clearInterval($.timer), await formatCookie($.checkLoginHeaders), $.done()) : 21 === checkRes.errcode ? ($.log("二维码已失效，请重新获取二维码重新扫描\n"), clearInterval($.timer), $.done()) : 176 === checkRes.errcode || ($.log(`扫描登录失败:其他未知状态异常：${JSON.stringify(checkRes)}\n`), clearInterval($.timer), $.done())
        } catch (e) {
            $.logErr(e)
        }
    }), 1e3)
...
function formatCookie(headers) {
    new Promise((resolve => {
        ...
        $.UserName = decodeURIComponent(cookie1.match(/pt_pin=([^; ]+)(?=;?)/) && cookie1.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.log(`京东用户：${$.UserName} Cookie获取成功，cookie如下：`), $.log(`\n${cookie1}\n`), resolve()
    }))
}

function formatSetCookies(headers, body) {
    new Promise((resolve => {
        s_token = body.s_token, guid = headers["set-cookie"][0], guid = guid.substring(guid.indexOf("=") + 1, guid.indexOf(";")), lsid = headers["set-cookie"][2], lsid = lsid.substring(lsid.indexOf("=") + 1, lsid.indexOf(";")), lstoken = headers["set-cookie"][3], lstoken = lstoken.substring(lstoken.indexOf("=") + 1, lstoken.indexOf(";")), cookies = "guid=" + guid + "; lang=chs; lsid=" + lsid + "; lstoken=" + lstoken + "; ", resolve()
    }))
}
...
function Env(t, e) {
    ...
    try {
        await loginEntrance(), await generateQrcode(), await getCookie()
    } catch (e) {
        $.logErr(e)
    }
})().catch((e => {
