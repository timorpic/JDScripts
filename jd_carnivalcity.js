...
const JD_API_HOST = "https://api.m.jd.com/api",
    activeEndTime = "2021/11/14 00:00:00+08:00";
...
function headInfo() {
    return new Promise((resolve => {
        $.post(taskUrl({
            apiMapping: "/khc/index/headInfo"
        }), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} headInfo API请求失败，请检查网路重试`)) : data && 200 === (data = JSON.parse(data)).code && ("0" === data.data.state ? "13" === data.data.taskType || "15" === data.data.taskType ? (console.log("开始 【顶部】浏览任务,需等待6秒"), await doBrowseHead(data.data.taskIndex, data.data.taskId, data.data.taskType)) : "14" === data.data.taskType && (console.log("开始 【顶部】加购任务"), await getHeadJoinPrize(data.data.taskId, data.data.taskIndex)) : $.stop = !0)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function doBrowseHead(taskIndex, taskId, taskType) {
    return new Promise((resolve => {
        const body = {
            ...
            taskId: taskId,
            taskType: taskType,
            apiMapping: "/khc/task/doBrowseHead"
        };
        $.post(taskUrl(body), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} doBrowseHead API请求失败，请检查网路重试`)) : data && 200 === (data = JSON.parse(data)).code && (await $.wait(6e3), await getHeadBrowsePrize(data.data.browseId))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function getHeadBrowsePrize(browseId) {
    return new Promise((resolve => {
        const body = {
            browseId: browseId,
            apiMapping: "/khc/task/getHeadBrowsePrize"
        };
        $.post(taskUrl(body), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} getHeadBrowsePrize API请求失败，请检查网路重试`)) : data && 200 === (data = JSON.parse(data)).code && console.log("getHeadBrowsePrize 领取奖励结果", JSON.stringify(data), "\n")
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function getHeadJoinPrize(taskId, taskIndex) {
    return new Promise((resolve => {
        const body = {
            taskId: taskId,
            taskIndex: taskIndex,
            apiMapping: "/khc/task/getHeadJoinPrize"
        };
        $.post(taskUrl(body), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} getHeadJoinPrize API请求失败，请检查网路重试`)) : data && 200 === (data = JSON.parse(data)).code && console.log("getHeadJoinPrize 领取奖励结果", JSON.stringify(data), "\n")
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function brandTaskInfo(brandId) {
    const body = {
        brandId: brandId,
        apiMapping: "/khc/index/brandTaskInfo"
    };
        ...
        $.get(taskUrl(body), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                    ...
                    for (let sku of $.skuTask.filter((vo => !!vo && "4" !== vo.status))) console.log(`\n开始做 品牌手机 【${data.data.brandName}】 任务`), console.log(`开始浏览 1-F 单品区 任务 ${sku.name}`), await doBrowse(sku.id, brandId, "brand", "presell", "browseSku"), await $.wait(6e3), $.browseId && await getBrowsePrize($.browseId, brandId);
                    for (let sku of $.shopTask.filter((vo => !!vo && "4" !== vo.status))) console.log(`\n开始做 品牌手机 【${data.data.brandName}】 任务`), console.log(`开始浏览 2-F 专柜区 任务 ${sku.name}，需等待10秒`), await doBrowse(sku.id, brandId, "brand", "follow", "browseShop"), await $.wait(10100), $.browseId && await getBrowsePrize($.browseId, brandId);
                    for (let sku of $.meetingTask.filter((vo => !!vo && "4" !== vo.status))) console.log(`\n开始做 品牌手机 【${data.data.brandName}】 任务`), console.log(`开始浏览 3-F 综合区 任务 ${sku.name}，需等待10秒`), await doBrowse(sku.id, brandId, "brand", "meeting", "browseVenue"), await $.wait(10500), $.browseId && await getBrowsePrize($.browseId, brandId);
                    if ($.questionTask.hasOwnProperty("id") && "0" === $.questionTask.result) {
                        console.log(`\n开始做 品牌手机 【${data.data.brandName}】 任务`), console.log(`开始做答题任务 ${$.questionTask.question}`);
                        let result = 0;
...
function doQuestion(brandId, questionId, result) {
    return new Promise((resolve => {
        const body = {
            ...
            result: result,
            apiMapping: "/khc/task/doQuestion"
        };
        $.post(taskUrl(body), ((err, resp, data) => {
            try {
...
function indexInfo(flag = !1) {
    const body = {
        apiMapping: "/khc/index/indexInfo"
    };
    return $.hotProductList = [], $.brandList = [], $.browseshopList = [], new Promise((resolve => {
        $.post(taskUrl(body), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : 200 === (data = JSON.parse(data)).code ? ($.hotProductList = data.data.hotProductList || [], $.brandList = data.data.brandList || [], $.browseshopList = data.data.browseshopList || []) : console.log(`异常：${JSON.stringify(data)}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function supportList() {
    const body = {
        apiMapping: "/khc/index/supportList"
    };
    return new Promise((resolve => {
        $.get(taskUrl(body), (async (err, resp, data) => {
            try {
...
function check() {
    const body = {
        pageNum: 1,
        apiMapping: "/khc/record/convertRecord"
    };
    return new Promise((resolve => {
        $.get(taskUrl(body), (async (err, resp, data) => {
            try {
...
function myRank() {
    return new Promise((resolve => {
        $.post(taskUrl({
            apiMapping: "/khc/rank/myPastRanks"
        }), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
...
function saveJbean(date) {
    return new Promise((resolve => {
        const body = {
            date: date,
            apiMapping: "/khc/rank/getRankJingBean"
        };
        $.post(taskUrl(body), ((err, resp, data) => {
            try {
...
function getListJbean() {
    return new Promise((resolve => {
        $.post(taskUrl({
            pageNum: "",
            apiMapping: "/khc/record/jingBeanRecord"
        }), (async (err, resp, data) => {
            try {
...
function getListIntegral() {
    return new Promise((resolve => {
        $.post(taskUrl({
            pageNum: "",
            apiMapping: "/khc/record/integralRecord"
        }), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                else if (200 === (data = JSON.parse(data)).code) {
...
function Env(t, e) {
    ...
    if (cookiesArr[0]) {
        ...
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                if (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.isLogin = !0, $.nickName = "", $.jingBeanNum = 0, $.integralCount = 0, $.integer = 0, $.lasNum = 0, $.num = 0, $.beans = 0, $.blockAccount = !1, message = "", await TotalBean(), console.log(`\n******开始【京东账号${$.index}】${$.nickName||$.UserName}*********\n`), !$.isLogin) {
