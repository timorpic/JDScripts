...
let notify = "",
    notifyLevel = $.isNode() && process.env.JXNC_NOTIFY_LEVEL || 0,
    notifyBool = !0,
    cookieArr = [],
    currentCookie = "",
    tokenNull = {
...
let allMessage = "";
$.detail = [], $.helpTask = null, $.allTask = [], $.info = {}, $.answer = 3, $.drip = 0, $.appId = 10016,
    function (t, r) {
    ...
    })), $.maxHelpNum = $.isNode() ? 8 : 4, $.helpNum = 0;
let assistUserShareCode = 0;
...
function requireConfig() {
    return new Promise((async resolve => {
        ...
        const jdCookieNode = $.isNode() ? require("./jdCookie.js") : "",
            jdJxncShareCodeNode = $.isNode() ? require("./jdJxncShareCodes.js") : {};
        $.isNode() ? (Object.keys(jdCookieNode).forEach((item => {
            jdCookieNode[item] && cookieArr.push(jdCookieNode[item])
        })), process.env.JD_DEBUG && "false" === process.env.JD_DEBUG && (console.log = () => {})) : cookieArr = [$.getdata("CookieJD"), $.getdata("CookieJD2"), ...jsonParse($.getdata("CookiesJD") || "[]").map((item => item.cookie))].filter((item => !!item)), $.log(`共${cookieArr.length}个京东账号\n`), $.isNode() && Object.keys(jdJxncShareCodeNode).forEach((item => {
            jdJxncShareCodeNode[item] ? jxncShareCodeArr.push(jdJxncShareCodeNode[item]) : jxncShareCodeArr.push("")
        }));
        for (let i = 0; i < jxncShareCodeArr.length; i++)
            if (jxncShareCodeArr[i]) {
                changeShareCodeJson(jxncShareCodeArr[i].split("@")[0]) || ($.log("互助码格式已变更，请重新填写互助码"), $.msg($.name, "互助码格式变更通知", "互助码格式变更，请重新填写 ‼️‼️", option), $.isNode() && await notify.sendNotify(`${$.name}`, "互助码格式变更，请重新填写 ‼️‼️"));
                break
            } $.log(`您提供了${jxncShareCodeArr.length}个账号的京喜农场助力码`);
        try {
            let options = {
                url: "https://sub.timor.icu/JDscripts/jxnc.txt",
                headers: {
...
function tokenFormat() {
    return new Promise((async resolve => {
        currentToken = tokenArr[$.index - 1] && tokenArr[$.index - 1].farm_jstoken ? tokenArr[$.index - 1] : tokenNull, resolve()
    }))
}

function shareCodesFormat() {
    return new Promise((async resolve => {
        jxncShareCodeArr[$.index - 1] ? (currentShareCode = jxncShareCodeArr[$.index - 1].split("@"), currentShareCode.push(...shareCode.split("@"))) : ($.log(`由于您第${$.index}个京东账号未提供shareCode,将采纳本脚本自带的助力码`), currentShareCode = shareCode.split("@")), $.log(`第${$.index}个京东账号将要助力的好友${JSON.stringify(currentShareCode)}`), resolve()
    }))
}
async function jdJXNC() {
    subTitle = `【京东账号${$.index}】${$.nickName}`, $.log("获取用户信息 & 任务列表");
    const startInfo = await getTaskList();
    if (startInfo)
        if (message += `【水果名称】${startInfo.prizename}\n`, startInfo.target <= startInfo.score) 2 === startInfo.activestatus ? (notifyBool = !0, $.log(`【成熟】水果已成熟请及时收取，activestatus：${startInfo.activestatus}\n`), message += `【成熟】水果已成熟请及时收取，activestatus：${startInfo.activestatus}\n`) : 0 === startInfo.activestatus && ($.log("账号未选择种子，请先去京喜农场选择种子。\n如果选择 APP 专属种子，必须提供 token。"), message += "账号未选择种子，请先去京喜农场选择种子。\n如果选择 APP 专属种子，必须提供 token。\n", notifyBool = notifyBool && notifyLevel >= 3);
        else {
            let shareCodeJson = {
                smp: $.info.smp,
                active: $.info.active,
            ...
            $.log(`【京东账号${$.index}（${$.UserName}）的${$.name}好友互助码】` + JSON.stringify(shareCodeJson)), await $.wait(500);
            if (await browserTask()) {
                await $.wait(500), await answerTask(), await $.wait(500);
                getMessage(await getTaskList(), startInfo), await submitInviteId($.UserName), await $.wait(500);
                let next = await helpFriends();
                if (next)
                    for (; $.helpNum < $.maxHelpNum && ($.helpNum++, assistUserShareCodeJson = await getAssistUser(), assistUserShareCodeJson && (await $.wait(500), next = await helpShareCode(assistUserShareCodeJson.smp, assistUserShareCodeJson.active, assistUserShareCodeJson.joinnum), next));) await $.wait(1e3)
            }
        } await showMsg()
}
        ...
        $.get(taskUrl("query", "type=1"), (async (err, resp, data) => {
            try {
                ...
                if (res) {
                    ...
                    $.detail = detail, $.helpTask = task.filter((x => 2 === x.tasktype))[0] || {
                        eachtimeget: 0,
                        limit: 0
                    }, $.allTask = task.filter((x => 3 !== x.tasktype && 2 !== x.tasktype && parseInt(x.left) > 0)), $.info = other, $.log(`获取任务列表 ${retmsg} 总共${$.allTask.length}个任务！`), $.info.active || ($.log("账号未选择种子，请先去京喜农场选择种子。\n如果选择 APP 专属种子，必须提供 token。"), message += "账号未选择种子，请先去京喜农场选择种子。\n如果选择 APP 专属种子，必须提供 token。\n", notifyBool = notifyBool && notifyLevel >= 3, resolve(!1)), resolve(other)
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
function browserTask() {
    return new Promise((async resolve => {
        const tasks = $.allTask.filter((x => 6 !== x.tasklevel)),
            times = Math.max(...[...tasks].map((x => x.limit)));
        for (let i = 0; i < tasks.length; i++) {
            ...
            for (let i = 0; i < times; i++) {
                ...
                if (await $.wait(1e3 * random), 0 === status[0] && (status[0] = await doTask(task)), 0 !== status[0]) break
            }
            if (1017 === status[0]) {
                $.log("水滴已满，果实成熟，跳过所有任务"), resolve(!0);
                break
            }
            if (1032 === status[0]) return $.log("任务执行失败，种植的 APP 专属种子，请提供 token 或种植非 APP 种子"), message += "任务执行失败，种植的 APP 专属种子，请提供 token 或种植非 APP 种子\n", notifyBool = notifyBool && notifyLevel >= 2, void resolve(!1);
...
function answerTask() {
    const _answerTask = $.allTask.filter((x => 6 === x.tasklevel));
    ...
    return $.log(`准备做答题任务：${taskname}`), new Promise((async resolve => {
        if (parseInt(left) <= 0) return resolve(!1), void $.log(`${taskname}[做任务]： 任务已完成，跳过`);
        $.get(taskUrl("dotask", `active=${$.info.active}&answer=${$.info.indexday}:${["A","B","C","D"][$.answer]}:0&joinnum=${$.info.joinnum}&tasklevel=${tasklevel}&_stk=${encodeURIComponent("active,answer,ch,farm_jstoken,joinnum,phoneid,tasklevel,timestamp")}`), (async (err, resp, data) => {
            try {
                let res = data.match(/try\{whyour\(([\s\S]*)\)\;\}catch\(e\)\{\}/);
                if (res) {
                    res = res[1];
                    ...
                    if (retmsg = "" !== retmsg ? retmsg : "成功", $.log(`${taskname}[做任务]：ret:${ret} retmsg:"${-1!==retmsg.indexOf("活动太火爆了")?"任务进行中或者未到任务时间":retmsg}"`), 0 === ret && 1 === right && ($.drip += eachtimeget), 1017 === ret || 1012 === ret) return void resolve();
                    (0 !== ret && 1029 !== ret || "ans err" === retmsg) && $.answer > 0 && ($.answer--, await $.wait(1e3), await answerTask())
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
function getMessage(endInfo, startInfo) {
    const need = endInfo.target - endInfo.score,
        get = endInfo.modifyscore,
        leaveGet = startInfo.modifyscore;
    let dayGet = 0;
    if ($.detail) {
        let dayTime = new Date((new Date).toLocaleDateString()).getTime() / 1e3;
        $.detail.forEach((function (item, index) {
            item.time >= dayTime && item.score && (dayGet += item.score)
        }))
    }
    if (message += `【水滴】本次获得${get} 离线获得${leaveGet} 今日获得${dayGet} 还需水滴${need}\n`, need <= 0) return notifyBool = !0, void(message += `【成熟】水果已成熟请及时收取，deliverState：${endInfo.deliverState}\n`);
    if (get > 0 || leaveGet > 0 || dayGet > 0) {
        const day = Math.ceil(need / (dayGet > 0 ? dayGet : get + leaveGet));
        message += `【预测】还需 ${day} 天\n`
    }
    notifyBool = get > 0 || leaveGet > 0 ? notifyBool && notifyLevel >= 1 : notifyBool && notifyLevel >= 2
}

function submitInviteId(userName) {
    return new Promise((resolve => {
        if ($.info && $.info.smp) try {
            $.post({
                ...
                try {
                    ...
                    $.log(`邀请码提交：${code}`), data.value && (message += "【邀请码】提交成功！\n")
                } catch (e) {
            ...
            $.get({
                ...
                try {
                    ...
                    if (value && extra.active) {
                        ...
                        return $.log("获取随机助力码成功 " + JSON.stringify(shareCodeJson)), void resolve(shareCodeJson)
                    }
                    $.log(`获取随机助力码失败 ${code}`)
                } catch (e) {
...
async function helpFriends() {
    ...
    for (let code of currentShareCode) {
        if (!code) continue;
        if (tmpShareCodeJson = changeShareCodeJson(code), !tmpShareCodeJson) {
            console.log("助力码非 json 格式，跳过");
            continue
        }
        if (!await helpShareCode(tmpShareCodeJson.smp, tmpShareCodeJson.active, tmpShareCodeJson.joinnum)) return !1;
        await $.wait(1e3)
    }
    return !0
}
...
function helpShareCode(smp, active, joinnum) {
    return new Promise((async resolve => {
        smp === $.info.smp && ($.log("助力码与当前账号相同，跳过助力。准备进行下一个助力"), resolve(!0)), $.log(`即将助力 share {"smp":"${smp}","active":"${active}","joinnum":"${joinnum}"}`), $.get(taskUrl("help", `active=${active}&joinnum=${joinnum}&smp=${smp}`), (async (err, resp, data) => {
            try {
                let res = data.match(/try\{whyour\(([\s\S]*)\)\;\}catch\(e\)\{\}/);
                if (res) {
                    res = res[1];
                    ...
                    if ($.log(`助力结果：ret=${ret} retmsg="${retmsg||"OK"}"`), 147 === ret || 1016 === ret) return 147 === ret && $.log("\n\n  !!!!!!!!   当前账号黑号了  !!!!!!!!  \n\n"), void resolve(!1);
                    resolve(!0)
                }
            } catch (e) {
                $.logErr(e, resp)
...
function doTask({
    ...
    return new Promise((async resolve => {
        parseInt(left) <= 0 && ($.log(`${taskname}[做任务]： 任务已完成，跳过`), resolve(!1)), $.get(taskUrl("dotask", `active=${$.info.active}&answer=${$.info.indexday}:D:0&joinnum=${$.info.joinnum}&tasklevel=${tasklevel}&_stk=${encodeURIComponent("active,answer,ch,farm_jstoken,joinnum,phoneid,tasklevel,timestamp")}`), ((err, resp, data) => {
            try {
                let res = data.match(/try\{whyour\(([\s\S]*)\)\;\}catch\(e\)\{\}/);
                if (res) {
                    res = res[1];
                    ...
                    retmsg = "" !== retmsg ? retmsg : "成功", $.log(`${taskname}[做任务]：ret:${ret} retmsg:"${-1!==retmsg.indexOf("活动太火爆了")?"任务进行中或者未到任务时间":retmsg}"`), 0 === ret && ($.drip += eachtimeget), resolve(ret)
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
async function showMsg() {
    notifyBool ? ($.msg($.name, subTitle, message, option), $.isNode() && (allMessage += `${subTitle}\n${message}${$.index!==cookieArr.length?"\n\n":""}`)) : $.log(`${$.name} - notify 通知已关闭\n账号${$.index} - ${$.nickName}\n${subTitle}\n${message}`)
}
...
function jsonParse(str) {
    ...
    if (await requireConfig(), cookieArr[0]) {
        $.CryptoJS = $.isNode() ? require("crypto-js") : CryptoJS, await requestAlgo();
        for (let i = 0; i < cookieArr.length; i++)
            if (cookieArr[i]) {
                if (currentCookie = cookieArr[i], $.UserName = decodeURIComponent(currentCookie.match(/pt_pin=([^; ]+)(?=;?)/) && currentCookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.isLogin = !0, $.nickName = "", $.log(`\n************* 检查【京东账号${$.index}】${$.UserName} cookie 是否有效 *************`), await TotalBean(), $.log(`开始【京东账号${$.index}】${$.nickName||$.UserName}\n`), !$.isLogin) {
                    $.msg($.name, "【提示】cookie已失效", `京东账号${$.index} ${$.nickName||$.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/`, {
                        "open-url": "https://bean.m.jd.com/"
                    }), $.isNode() && await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
                    continue
                }
                currentCookie.includes("pt_pin") && await getJxToken(), subTitle = "", message = "", option = {}, $.answer = 3, $.helpNum = 0, notifyBool = notifyLevel > 0, await shareCodesFormat(), await jdJXNC()
            } $.isNode() && allMessage && await notify.sendNotify(`${$.name}`, `${allMessage}`)
    } else $.msg($.name, "【提示】请先获取京东账号一cookie\n直接使用NobyDa的京东签到获取", "https://bean.m.jd.com/bean/signIndex.action", {
        "open-url": "https://bean.m.jd.com/bean/signIndex.action"
    })
})().catch((e => {
    $.log("", `❌ ${$.name}, 失败! 原因: ${e}!`, ""), console.log(e)
})).finally((() => {
    $.done()
...
var _0xodF = "jsjiami.com.v6",
    _0x5b0b = [_0xodF, "cyNU", "XcOkE8O8wqc=", "bsOsw4LCsRUKacOqw6xywrnCpcOBwqzDrMK4w78Hc0RtARs1LMK7UsORw5DDtTvDlMOhTWdJ", "w7PDhsK5LTw=", "VA0TwroQwpk=", "w6xQw4hObsKK", "wqrDmcK/KmQ=", "wqduwp7DhcOT", "KCpb", "5q2F6Laf5Y+2w69KNcKRQ8Ki5aGE5Ye45Lug6KS56Iy4dOS9lueZvcKJHWtDelV+S8KQwqHlkZDpnpnmsrHli4jljpnDpQAMw7A=", "qzJQKjsjhUKiamhDLiI.Jrcom.v6=="];
! function (_0x575984, _0x111392, _0x45a010) {
    (function (_0x3fe5bc, _0x4fea58, _0x564d83, _0x7e4c96, _0x31f329) {
        if ((_0x4fea58 >>= 8) < _0x3fe5bc) {
            for (; --_0x3fe5bc;) _0x7e4c96 = _0x575984.shift(), _0x4fea58 === _0x3fe5bc ? (_0x4fea58 = _0x7e4c96, _0x564d83 = _0x575984.pop()) : _0x4fea58 && _0x564d83.replace(/[qzJQKhUKhDLIJr=]/g, "") === _0x4fea58 && _0x575984.push(_0x7e4c96);
            _0x575984.push(_0x575984.shift())
        }
    })(++_0x111392, 79872)
}(_0x5b0b, 312);
var _0xa80f = function (_0x573aaa, _0x1954c9) {
    _0x573aaa = ~~"0x".concat(_0x573aaa);
    var _0x28dbcb = _0x5b0b[_0x573aaa];
    if (void 0 === _0xa80f.fYTBaI) {
        ! function () {
            var _0x34fb78 = "undefined" != typeof window ? window : "object" == typeof process && "function" == typeof require && "object" == typeof global ? global : this;
            _0x34fb78.atob || (_0x34fb78.atob = function (_0x108344) {
                for (var _0x409c1c, _0x79d606, _0x1c9d23 = String(_0x108344).replace(/=+$/, ""), _0x231b41 = 0, _0x4d4dd3 = 0, _0x539f42 = ""; _0x79d606 = _0x1c9d23.charAt(_0x4d4dd3++); ~_0x79d606 && (_0x409c1c = _0x231b41 % 4 ? 64 * _0x409c1c + _0x79d606 : _0x79d606, _0x231b41++ % 4) ? _0x539f42 += String.fromCharCode(255 & _0x409c1c >> (-2 * _0x231b41 & 6)) : 0) _0x79d606 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".indexOf(_0x79d606);
                return _0x539f42
            })
        }();
        _0xa80f.LuAVWF = function (_0x4969ce, _0x1954c9) {
            for (var _0x11157a, _0x1deab0 = [], _0x54f040 = 0, _0x28c8dd = "", _0x2e4762 = "", _0x4c665e = 0, _0x81f3b1 = (_0x4969ce = atob(_0x4969ce)).length; _0x4c665e < _0x81f3b1; _0x4c665e++) _0x2e4762 += "%" + ("00" + _0x4969ce.charCodeAt(_0x4c665e).toString(16)).slice(-2);
            _0x4969ce = decodeURIComponent(_0x2e4762);
            for (var _0x4c705a = 0; _0x4c705a < 256; _0x4c705a++) _0x1deab0[_0x4c705a] = _0x4c705a;
            for (_0x4c705a = 0; _0x4c705a < 256; _0x4c705a++) _0x54f040 = (_0x54f040 + _0x1deab0[_0x4c705a] + _0x1954c9.charCodeAt(_0x4c705a % _0x1954c9.length)) % 256, _0x11157a = _0x1deab0[_0x4c705a], _0x1deab0[_0x4c705a] = _0x1deab0[_0x54f040], _0x1deab0[_0x54f040] = _0x11157a;
            _0x4c705a = 0, _0x54f040 = 0;
            for (var _0x4f3ecc = 0; _0x4f3ecc < _0x4969ce.length; _0x4f3ecc++) _0x54f040 = (_0x54f040 + _0x1deab0[_0x4c705a = (_0x4c705a + 1) % 256]) % 256, _0x11157a = _0x1deab0[_0x4c705a], _0x1deab0[_0x4c705a] = _0x1deab0[_0x54f040], _0x1deab0[_0x54f040] = _0x11157a, _0x28c8dd += String.fromCharCode(_0x4969ce.charCodeAt(_0x4f3ecc) ^ _0x1deab0[(_0x1deab0[_0x4c705a] + _0x1deab0[_0x54f040]) % 256]);
            return _0x28c8dd
        }, _0xa80f.mCXDyr = {}, _0xa80f.fYTBaI = !0
    }
    var _0x390a3d = _0xa80f.mCXDyr[_0x573aaa];
    return void 0 === _0x390a3d ? (void 0 === _0xa80f.dUKlOc && (_0xa80f.dUKlOc = !0), _0x28dbcb = _0xa80f.LuAVWF(_0x28dbcb, _0x1954c9), _0xa80f.mCXDyr[_0x573aaa] = _0x28dbcb) : _0x28dbcb = _0x390a3d, _0x28dbcb
};

function getJxToken() {
    var _0xa422 = {
        lGSBc: _0xa80f("0", "9qEv"),
    ...
    function _0x2cfb27(_0x3d541e) {
        let _0x19600a = _0xa422[_0xa80f("1", "Ye&F")],
            _0x1a62e9 = "";
        for (let _0x44ee85 = 0; _0x44ee85 < _0x3d541e; _0x44ee85++) _0x1a62e9 += _0x19600a[parseInt(Math[_0xa80f("2", "]aCG")]() * _0x19600a[_0xa80f("3", "J@ZI")])];
        return _0x1a62e9
    }
    return new Promise((_0x3d4dca => {
        let _0x1934d2 = _0xa422[_0xa80f("4", "&w[x")](_0x2cfb27, 40),
            _0x140a50 = (+new Date).toString();
        currentCookie[_0xa80f("5", "YcXf")](/pt_pin=([^; ]+)(?=;?)/) || (console[_0xa80f("6", "WGWr")](_0xa80f("7", "4!)U")), _0x3d4dca(null));
        let _0x1ab949 = currentCookie.match(/pt_pin=([^; ]+)(?=;?)/)[1],
            _0xa37fe3 = $[_0xa80f("8", "MVeX")]("" + _0xa422.EtUUR(decodeURIComponent, _0x1ab949) + _0x140a50 + _0x1934d2 + "tPOamqCuk9NLgVPAljUyIHcPRmKlVxDy").toString();
        currentToken = {
            timestamp: _0x140a50,
            phoneid: _0x1934d2,
            farm_jstoken: _0xa37fe3
        }, _0xa422[_0xa80f("9", "ZsI%")](_0x3d4dca)
    }))
}
