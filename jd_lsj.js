...
let lsjdh = "";
async function star() {
    await gettoken(), $.log("开始入会任务"), await dojoinMember(1000101562), await $.wait(3e3), await dojoinMember(1000077335), await $.wait(3e3), await dojoinMember(1000008814), await $.wait(3e3), await dojoinMember(1000014803), $.log("开始领取首页水滴"), await dotree(1), await $.wait(3e3), await dotree(2), await $.wait(3e3), await dotree(3), await $.wait(3e3), $.log("开始浏览会场"), await doliulan(1), await $.wait(3e3), await doliulan(2), await $.wait(3e3), await doliulan(3), $.log("开始浏览会场"), await doshop(1000014803), await $.wait(3e3), await doshop(10299171), await $.wait(3e3), await doshop(1000077335), await $.wait(3e3), await doshop(1000008814), await $.wait(3e3), await doshop(1000101562), $.log("开始浏览推荐食品商品"), await doGoods(1), await $.wait(3e3), await doGoods(2), await $.wait(3e3), await doGoods(3), await $.wait(3e3), await doGoods(4), $.log("开始加购商品"), await doadd(1), await $.wait(3e3), await doadd(2), await $.wait(3e3), await doadd(3), await $.wait(3e3), await doadd(4), $.log("开始游戏刷分"), await playgame(), $.log("开始兑换"), await duihuan()
}
    ...
    return new Promise((async resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                ...
                0 == reust.errcode ? (token = reust.token, $.log(token), await getnick()) : $.log(data)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function getnick() {
    return new Promise((async resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                const reust = JSON.parse(data);
                200 == reust.errorCode ? (nick = reust.data.data.msg, $.log("邀请码: " + nick), useInfo[$.UserName] = nick, newShareCodes.push({
                    usr: $.UserName,
                    code: nick,
                    max: !1
                })) : 500 == reust.errorCode && $.log(reust.errorMessage)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function doshop(goodsNumId) {
    return new Promise((async resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                const reust = JSON.parse(data);
                200 == reust.errorCode ? $.log(`${reust.data.data.remark}\n获得${reust.data.data.sendNum}`) : 500 == reust.errorCode && $.log("今日已领取完毕,请明日再来！" + reust.errorMessage)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
    ...
    return new Promise((async resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                console.log(data);
                const reust = JSON.parse(data);
                200 == reust.errorCode ? ("好友助力数量已达上限，无法为好友助力！" === reust.data.data.remark ? $.oneCodeInfo.max = !0 : $.canHelp = !1, $.log(`${reust.data.data.remark}`)) : 500 == reust.errorCode && $.log(reust.errorMessage)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function dojoinMember(id) {
    return new Promise((async resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                const reust = JSON.parse(data);
                200 == reust.errorCode ? ($.log("\n如果入会失败 请手动去入会\n"), $.log(`${reust.data.data.remark}`)) : 500 == reust.errorCode && $.log(reust.errorMessage)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function playgame() {
    return new Promise((async resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                const reust = JSON.parse(data);
                200 == reust.errorCode ? 1 == reust.data.data.enoughCoin ? ($.log(`刷分成功 刷金币成功${reust.data.data.point} 正在前往领取京豆`), await ljd("jdRunningBox1"), await $.wait(3e3), await ljd("jdRunningBox2"), await $.wait(3e3), await ljd("jdRunningBox3")) : 0 == reust.data.data.enoughCoin && $.log(`${reust.data.data.msg}`) : 500 == reust.errorCode && $.log(reust.errorMessage)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function ljd(awardId) {
    return new Promise((async resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                const reust = JSON.parse(data);
                200 == reust.errorCode ? (jdbean = reust.data.data.msg, $.log(`${reust.data.data.msg}`), await showMsg()) : 500 == reust.errorCode && $.log(reust.errorMessage)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function duihuan() {
    return new Promise((async resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                const reust = JSON.parse(data);
                200 == reust.errorCode ? $.log(`${reust.data.data.msg}`) : 500 == reust.errorCode && $.log("今日已领取完毕,请明日再来！" + reust.errorMessage)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
process.env.lsjdh && (lsjdh = process.env.lsjdh), $.isNode() ? (Object.keys(jdCookieNode).forEach((item => {
    ...
    if (cookiesArr[0]) {
        ...
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), useInfo[$.UserName]) {
                $.canHelp = !0;
                for (let j = 0; j < newShareCodes.length && $.canHelp; j++)
                    if ($.oneCodeInfo = newShareCodes[j], $.UserName !== newShareCodes[j].usr && !$.oneCodeInfo.max)
                        for (let k = 0; k < $.ShareCode.length && $.canHelp; k++) $.oneCodeInfo = $.ShareCode[k], $.UserName === $.ShareCode[k] || $.oneCodeInfo.max || (console.log(`${$.UserName}去助力${newShareCodes[j].usr}`), console.log(`${$.UserName}去助力${$.ShareCode}`), nick = useInfo[$.UserName], await dohelp(newShareCodes[j].code), await $.wait(3e3))
            }
