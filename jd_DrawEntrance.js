const $ = new Env("天天优惠大乐透_Timorpic"),
...
function extend() {
    return new Promise((resolve => {
        $.post(taskUrl("getLuckyDrawResourceConfig", {
            ...
            try {
                err ? (console.log(err), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (data = JSON.parse(data), $.extend = data.result.luckyDrawConfig.extend)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
    ...
    return new Promise((resolve => {
        $.post(taskUrl("doLuckDrawEntrance", body), (async (err, resp, data) => {
            try {
                ...
                else if (safeGet(data) && (data = JSON.parse(data)).success && "luckyDrawData" in data.result)
                    if (data.result.luckyDrawData.checkWinOrNot) switch (data.result.luckyDrawData.couponType) {
                        case "2":
                            console.log(`   成功领取优惠券：${data.result.luckyDrawData.discount}\n   ${data.result.luckyDrawData.quotaDesc}，${data.result.luckyDrawData.prizeName}`), allMessage += `\n【京东账号${$.index}】${$.nickName||$.UserName} \n          ${data.result.luckyDrawData.discount}\n   ${data.result.luckyDrawData.quotaDesc}，${data.result.luckyDrawData.prizeName}`;
                            break;
                        case "0":
                            console.log(`   成功领取无门槛红包：${data.result.luckyDrawData.quota}`), allMessage += `\n【京东账号${$.index}】${$.nickName||$.UserName} \n          成功领取无门槛红包：${data.result.luckyDrawData.quota}`;
                            break;
                        default:
                            console.log(JSON.stringify(data))
                    } else $.noChance = !0, console.log("已经没有次数了"), allMessage += `\n【京东账号${$.index}】${$.nickName||$.UserName} \n          已经没有次数了\n`
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
    ...
    if (cookiesArr[0]) {
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                ...
                $.ADID = getUUID("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", 1), $.UUID = getUUID("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"), $.noChance = !1, await extend();
                for (let x = 0; x < 3; x++) $.noChance || ($.log("\n尝试抽奖"), await doLuckDrawEntrance(), await $.wait(1e3))
