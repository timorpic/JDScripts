...
const $ = new Env("京喜工厂_timorpic"),
    JD_API_HOST = "https://m.jingxi.com",
    helpAu = !0,
    notify = $.isNode() ? require("./sendNotify") : "";
let notifyLevel = $.isNode() && process.env.JXGC_NOTIFY_LEVEL || 2;
const randomCount = $.isNode() ? 1 : 0;
let tuanActiveId = "",
    hasSend = !1;
...
let cookiesArr = [],
    ...
    message = "",
    allMessage = "";
const inviteCodes = ["bL_LSr3rcUFgsnwSECkbTQ==@04Z2GKwtQvLsLupbyOUEaA==", "bL_LSr3rcUFgsnwSECkbTQ==@04Z2GKwtQvLsLupbyOUEaA=="],
    jdCookieNode = $.isNode() ? require("./jdCookie.js") : "";
async function jdDreamFactory() {
    try {
        if (await userInfo(), await QueryFriendList(), await helpFriends(), !$.unActive) return;
        await getUserElectricity(), await taskList(), await investElectric(), await QueryHireReward(), tuanActiveId && (await tuanActivity(), await QueryAllTuan()), await exchangeProNotify(), await showMsg()
    } catch (e) {
        $.logErr(e)
    }
}
    ...
    return new Promise((async resolve => {
        ...
        $.get(options, (async (err, resp, data) => {
            try {
                ...
                else if (data = data && data.match(/window\._CONFIG = (.*) ;var __getImgUrl/)) {
                    const tuanConfigs = ((data = JSON.parse(data[1]))[0].skinConfig[0].adConfig || []).filter((vo => !!vo && "h5" === vo.channel));
                        ...
                        for (let item of tuanConfigs) {
                            ...
                            if (new Date(item.start).getTime() <= Date.now() && new Date(item.end).getTime() > Date.now()) {
                                if (link && link.match(/activeId=(.*),/) && link.match(/activeId=(.*),/)[1]) {
                                    console.log(`\n团活动ID: ${link.match(/activeId=(.*),/)[1]}\n有效时间：${start} - ${end}`), tuanActiveId = link.match(/activeId=(.*),/)[1];
                                    break
                                }
                            } else new Date(item.start).getTime() > Date.now() && new Date(item.end).getTime() > Date.now() ? link && link.match(/activeId=(.*),/) && link.match(/activeId=(.*),/)[1] && (console.log(`\n团活动ID: ${link.match(/activeId=(.*),/)[1]}\n有效时间：${start} - ${end}\n团ID还未开始`), tuanActiveId = "") : link && link.match(/activeId=(.*),/) && link.match(/activeId=(.*),/)[1] && (console.log(`\n团活动ID: ${link.match(/activeId=(.*),/)[1]}\n有效时间：${start} - ${end}\n团ID已过期`), tuanActiveId = "")
                        }
                }
            } catch (e) {
                $.logErr(e, resp)
...
function collectElectricity(facId = $.factoryId, help = !1, master) {
    return new Promise((async resolve => {
        let body = `factoryid=${facId}&apptoken=&pgtimestamp=&phoneID=&doubleflag=1`;
        help && master && (body += `factoryid=${facId}&master=${master}`), $.get(taskurl("generator/CollectCurrentElectricity", body, "_time,apptoken,doubleflag,factoryid,pgtimestamp,phoneID,timeStamp,zone"), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (0 === (data = JSON.parse(data)).ret ? help ? ($.ele += Number(data.data.loginPinCollectElectricity), console.log(`帮助好友收取 ${data.data.CollectElectricity} 电力，获得 ${data.data.loginPinCollectElectricity} 电力`), message += `【帮助好友】帮助成功，获得 ${data.data.loginPinCollectElectricity} 电力\n`) : ($.ele += Number(data.data.CollectElectricity), console.log(`收取电力成功: 共${data.data.CollectElectricity} `), message += `【收取发电站】收取成功，获得 ${data.data.CollectElectricity} 电力\n`) : help ? console.log(`收取好友电力失败:${data.msg}\n`) : console.log(`收取电力失败:${data.msg}\n`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function investElectric() {
    return new Promise((async resolve => {
        $.get(taskurl("userinfo/InvestElectric", `productionId=${$.productionId}`, "_time,productionId,zone"), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (0 === (data = JSON.parse(data)).ret ? (console.log(`成功投入电力${data.data.investElectric}电力`), message += `【投入电力】投入成功，共计 ${data.data.investElectric} 电力\n`) : (console.log(`投入失败，${data.msg}`), message += `【投入电力】投入失败，${data.msg}\n`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
        ...
        $.get(newtasksysUrl("GetUserTaskStatusList", "", "_time,bizCode,source"), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                    ...
                    let userTaskStatusList = (data = JSON.parse(data)).data.userTaskStatusList;
                    for (let i = 0; i < userTaskStatusList.length; i++) {
                        const vo = userTaskStatusList[i];
                        if (1 !== vo.awardStatus)
                            if (vo.completedTimes >= vo.targetTimes) console.log(`任务：${vo.description}可完成`), await completeTask(vo.taskId, vo.taskName), await $.wait(1e3);
                            else switch (vo.taskType) {
                                case 2:
                                case 6:
                                case 9:
                                    for (let i = vo.completedTimes; i <= vo.configTargetTimes; ++i) console.log(`去做任务：${vo.taskName}`), await doTask(vo.taskId), await completeTask(vo.taskId, vo.taskName), await $.wait(1e3)
                            }
                    }
                    console.log(`完成任务：共领取${$.ele}电力`), message += `【每日任务】领奖成功，共计 ${$.ele} 电力\n`
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
function getUserElectricity() {
    return new Promise((async resolve => {
        $.get(taskurl("generator/QueryCurrentElectricityQuantity", `factoryid=${$.factoryId}`, "_time,factoryid,zone"), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && 0 === (data = JSON.parse(data)).ret && (console.log(`发电机：当前 ${data.data.currentElectricityQuantity} 电力，最大值 ${data.data.maxElectricityQuantity} 电力`), data.data.currentElectricityQuantity < data.data.maxElectricityQuantity && $.log(`\n本次发电机电力集满分享后${1===data.data.nextCollectDoubleFlag?"可":"不可"}获得双倍电力，${1===data.data.nextCollectDoubleFlag?"故目前不收取电力":"故现在收取电力"}\n`), 1 === data.data.nextCollectDoubleFlag ? data.data.currentElectricityQuantity === data.data.maxElectricityQuantity && data.data.doubleElectricityFlag ? (console.log("发电机：电力可翻倍并收获"), await collectElectricity()) : message += `【发电机电力】当前 ${data.data.currentElectricityQuantity} 电力，未达到收获标准\n` : await collectElectricity())
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
            ...
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                    ...
                    if (0 === (data = JSON.parse(data)).ret)
                        for (let item of data.data.hireReward) item.date !== new Date((new Date).getTime() + 60 * (new Date).getTimezoneOffset() * 1e3 + 288e5).Format("yyyyMMdd") && await hireAward(item.date, item.type);
                    else console.log(`异常：${JSON.stringify(data)}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function hireAward(date, type = 0) {
    return new Promise((async resolve => {
        $.get(taskurl("friend/HireAward", `date=${date}&type=${type}`, "_time,date,type,zone"), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (0 === (data = JSON.parse(data)).ret ? (console.log("打工电力：收取成功"), message += "【打工电力】：收取成功\n") : (console.log(`打工电力：收取失败，${data.msg}`), message += `【打工电力】收取失败，${data.msg}\n`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
async function helpFriends() {
    if (new Date((new Date).getTime() + 60 * (new Date).getTimezoneOffset() * 1e3 + 288e5).getHours() < 6) console.log("\n未到招工时间(每日6-24点之间可招工)\n");
    else if ($.canHelpFlag) {
        ...
        for (let code of $.newShareCodes)
            if (code) {
                ...
                if (assistFriendRes && 0 === assistFriendRes.ret) {
                    console.log(`助力朋友：${code}成功，因一次只能助力一个，故跳出助力`);
                    break
                }
                if (assistFriendRes && 11009 === assistFriendRes.ret) {
            ...
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                    ...
                    if (0 === (data = JSON.parse(data)).ret) {
                        data = data.data;
                        const {
                            assistListToday: assistListToday = [],
                            assistNumMax: assistNumMax,
                            hireListToday: hireListToday = [],
                            hireNumMax: hireNumMax
                        } = data;
                        console.log(`\n\n你今日还能帮好友打工（${assistNumMax-assistListToday.length||0}/${assistNumMax}）次\n\n`), assistListToday.length === assistNumMax && ($.canHelpFlag = !1), $.log(`【今日招工进度】${hireListToday.length}/${hireNumMax}`), message += `【招工进度】${hireListToday.length}/${hireNumMax}\n`
                    } else console.log(`QueryFriendList异常：${JSON.stringify(data)}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function completeTask(taskId, taskName) {
    return new Promise((async resolve => {
        $.get(newtasksysUrl("Award", taskId, "_time,bizCode,source,taskId"), ((err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                else if (safeGet(data)) switch ((data = JSON.parse(data)).data.awardStatus) {
                    case 1:
...
function doTask(taskId) {
    return new Promise((async resolve => {
        $.get(newtasksysUrl("DoTask", taskId, "_time,bizCode,configExtra,source,taskId"), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (0 === (data = JSON.parse(data)).ret ? console.log("做任务完成！") : console.log(`DoTask异常：${JSON.stringify(data)}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
                ...
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                    ...
                    if (0 === (data = JSON.parse(data)).ret)
                        if (data = data.data, $.unActive = !0, $.encryptPin = "", $.shelvesList = [], data.factoryList && data.productionList) {
                            ...
                            $.factoryId = factory.factoryId, $.productionId = production.productionId, $.commodityDimId = production.commodityDimId, $.encryptPin = data.user.encryptPin, await GetCommodityDetails(), 1 === productionStage.productionStageAwardStatus ? ($.log("可以开红包了\n"), await DrawProductionStagePrize()) : $.log(`再加${productionStage.productionStageProgress}电力可开红包\n`), console.log(`当前电力：${data.user.electric}`), console.log(`当前等级：${data.user.currentLevel}`), console.log(`\n【京东账号${$.index}（${$.UserName}）的${$.name}好友互助码】${data.user.encryptPin}`), console.log(`已投入电力：${production.investedElectric}`), console.log(`所需电力：${production.needElectric}`), console.log(`生产进度：${(production.investedElectric/production.needElectric*100).toFixed(2)}%`), message += `【京东账号${$.index}】${$.nickName}\n`, message += `【生产商品】${$.productName}\n`, message += `【当前等级】${data.user.userIdentity} ${data.user.currentLevel}\n`, message += `【生产进度】${(production.investedElectric/production.needElectric*100).toFixed(2)}%\n`, production.investedElectric >= production.needElectric ? (1 === production.exchangeStatus && $.log("\n\n可以兑换商品了"), 3 === production.exchangeStatus && ($.log("\n\n商品兑换已超时"), 9 === (new Date).getHours() && ($.msg($.name, "", `【京东账号${$.index}】${$.nickName}\n【生产商品】${$.productName}兑换已超时，请选择新商品进行制造`), "3" != `${notifyLevel}` && "2" != `${notifyLevel}` || (allMessage += `【京东账号${$.index}】${$.nickName}\n【生产商品】${$.productName}兑换已超时，请选择新商品进行制造${$.index!==cookiesArr.length?"\n\n":""}`)))) : console.log(`\n\n预计最快还需 【${((production.needElectric-production.investedElectric)/172800).toFixed(2)}天】生产完毕\n\n`), 3 === production.status && ($.log("\n\n商品生产已失效"), $.msg($.name, "", `【京东账号${$.index}】${$.nickName}\n【生产商品】${$.productName}\n【超时未完成】已失效，请选择新商品进行制造`), !$.isNode() || "3" != `${notifyLevel}` && "2" != `${notifyLevel}` || (allMessage += `【京东账号${$.index}】${$.nickName}\n【生产商品】${$.productName}\n【超时未完成】已失效，请选择新商品进行制造${$.index!==cookiesArr.length?"\n\n":""}`))
                        } else if ($.unActive = !1, data.factoryList) {
                    if (data.factoryList && !data.productionList) {
                        console.log(`【提示】京东账号${$.index}[${$.nickName}]京喜工厂未选购商品\n请手动去京东APP->游戏与互动->查看更多->京喜工厂 选购\n`), 12 === new Date((new Date).getTime() + 60 * (new Date).getTimezoneOffset() * 1e3 + 288e5).getHours() && ($.msg($.name, "提醒⏰", `京东账号${$.index}[${$.nickName}]京喜工厂未选择商品\n请手动去京东APP->游戏与互动->查看更多->京喜工厂 选择商品`), $.isNode() && "3" == `${notifyLevel}` && (allMessage += `京东账号${$.index}[${$.nickName}]京喜工厂未选择商品\n请手动去京东APP->游戏与互动->查看更多->京喜工厂 选择商品${$.index!==cookiesArr.length?"\n\n":""}`))
                    }
                } else console.log(`【提示】京东账号${$.index}[${$.nickName}]京喜工厂活动未开始\n请手动去京东APP->游戏与互动->查看更多->京喜工厂 开启活动\n`);
...
function GetCommodityDetails() {
    return new Promise((async resolve => {
        $.get(taskurl("diminfo/GetCommodityDetails", `commodityId=${$.commodityDimId}`, "_time,commodityId,zone"), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (0 === (data = JSON.parse(data)).ret ? (data = data.data, $.productName = data.commodityList[0].name) : console.log(`GetCommodityDetails异常：${JSON.stringify(data)}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
            ...
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                    ...
                    if (0 === (data = JSON.parse(data)).ret) {
                        ...
                        shelvesList && ($.shelvesList = [...$.shelvesList, ...shelvesList], GetShelvesList(++pageNo))
                    } else console.log(`GetShelvesList异常：${JSON.stringify(data)}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function DrawProductionStagePrize() {
    return new Promise((async resolve => {
        $.get(taskurl("userinfo/DrawProductionStagePrize", `productionId=${$.productionId}`, "_time,productionId,zone"), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : console.log(`开幸运红包：${data}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
async function PickUp(encryptPin = $.encryptPin, help = !1) {
    $.pickUpMyselfComponent = !0;
    const GetUserComponentRes = await GetUserComponent(encryptPin, 1500);
    if (GetUserComponentRes && 0 === GetUserComponentRes.ret && GetUserComponentRes.data) {
        const {
            componentList: componentList
        } = GetUserComponentRes.data;
        componentList && componentList.length <= 0 && (help ? $.log(`好友【${encryptPin}】地下暂无零件可收\n`) : $.log("自家地下暂无零件可收\n"), $.pickUpMyselfComponent = !1);
        for (let item of componentList) {
            await $.wait(1e3);
            const PickUpComponentRes = await PickUpComponent(item.placeId, encryptPin);
            if (PickUpComponentRes) {
                if (0 !== PickUpComponentRes.ret) {
                    help ? console.log(`收好友[${encryptPin}]零件失败：${PickUpComponentRes.msg},直接跳出\n`) : (console.log(`收自己地下零件失败：${PickUpComponentRes.msg},直接跳出\n`), $.pickUpMyselfComponent = !1);
                    break
                } {
                    const data = PickUpComponentRes.data;
                    help ? (console.log(`收取好友[${encryptPin}]零件成功:获得${data.increaseElectric}电力\n`), $.pickFriendEle += data.increaseElectric) : (console.log(`收取自家零件成功:获得${data.increaseElectric}电力\n`), $.pickEle += data.increaseElectric)
                }
            }
        }
    }
    ...
    return new Promise((resolve => {
        setTimeout((() => {
            $.get(taskurl("usermaterial/GetUserComponent", `pin=${pin}`, "_time,pin,zone"), ((err, resp, data) => {
                try {
                    err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (0 === (data = JSON.parse(data)).ret || console.log(`GetUserComponent失败：${JSON.stringify(data)}`))
                } catch (e) {
                    $.logErr(e, resp)
                } finally {
                    resolve(data)
...
async function stealFriend() {
    if (1 === (new Date).getHours() || 12 === (new Date).getHours() || 19 === (new Date).getHours()) {
        await getFriendList(), $.friendList = [...new Set($.friendList)].filter((vo => !!vo && 1 !== vo.newFlag)), console.log(`查询好友列表完成，共${$.friendList.length}好友，下面开始拾取好友地下的零件\n`);
        for (let i = 0; i < $.friendList.length; i++) {
            let pin = $.friendList[i].encryptPin;
            console.log(`\n开始收取第 ${i+1} 个好友 【${$.friendList[i].nickName}】 地下零件 collectFlag：${$.friendList[i].collectFlag}`), await PickUp(pin, !0)
        }
    }
}
        ...
        $.get(taskurl("friend/QueryFactoryManagerList", `sort=${sort}`, "_time,sort,zone"), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                    ...
                    if (0 === (data = JSON.parse(data)).ret) {
                        if ((data = data.data).list && data.list.length <= 0) return;
                        let friendsEncryptPins = [];
                        for (let item of data.list) friendsEncryptPins.push(item);
                        $.friendList = [...$.friendList, ...friendsEncryptPins], await getFriendList(data.sort)
                    } else console.log(`QueryFactoryManagerList异常：${JSON.stringify(data)}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function getFactoryIdByPin(pin) {
    return new Promise(((resolve, reject) => {
        $.get(taskurl("userinfo/GetUserInfoByPin", `pin=${pin}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (0 === (data = JSON.parse(data)).ret ? data.data.factoryList && ($.stealFactoryId = data.data.factoryList[0].factoryId) : console.log(`异常：${JSON.stringify(data)}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
async function tuanActivity() {
    ...
    if (tuanConfig && 0 === tuanConfig.ret) {
        const {
            activeId: activeId,
            surplusOpenTuanNum: surplusOpenTuanNum,
            tuanId: tuanId
        } = tuanConfig.data.userTuanInfo;
        if (console.log(`今日剩余开团次数：${surplusOpenTuanNum}次`), $.surplusOpenTuanNum = surplusOpenTuanNum, !tuanId && surplusOpenTuanNum > 0) $.log("准备开团"), await CreateTuan();
        else if (tuanId) {
            const QueryTuanRes = await QueryTuan(activeId, tuanId);
            if (QueryTuanRes && 0 === QueryTuanRes.ret) {
                const {
                    tuanInfo: tuanInfo
                } = QueryTuanRes.data;
                if ((tuanInfo && tuanInfo[0].endTime) <= QueryTuanRes.nowTime && surplusOpenTuanNum > 0) return $.log("之前的团已过期，准备重新开团\n"), void await CreateTuan();
                for (let item of tuanInfo) {
                    const {
                        realTuanNum: realTuanNum,
                        tuanNum: tuanNum,
                        userInfo: userInfo
                    } = item;
                    if ($.tuanNum = tuanNum || 0, $.log(`\n开团情况:${realTuanNum}/${tuanNum}\n`), realTuanNum === tuanNum)
                        for (let user of userInfo) user.encryptPin === $.encryptPin && (user.receiveElectric && user.receiveElectric > 0 ? (console.log(`您在${new Date(1e3*user.joinTime).toLocaleString()}开团奖励已经领取成功\n`), $.surplusOpenTuanNum > 0 && await CreateTuan()) : ($.log("开始领取开团奖励"), await tuanAward(item.tuanActiveId, item.tuanId)));
                    else $.tuanIds.push(tuanId), $.log(`\n此团未达领取团奖励人数：${tuanNum}人\n`)
                }
            }
        }
    }
...
async function joinLeaderTuan() {
    let res = await updateTuanIdsCDN("http://sub.timor.icu/JDscripts/jd_updateFactoryTuanId.json");
    if (res || (res = await updateTuanIdsCDN("http://sub.timor.icu/JDscripts/jd_updateFactoryTuanId.json")), $.authorTuanIds = [...res && res.tuanIds || []], $.authorTuanIds && $.authorTuanIds.length)
        for (let tuanId of $.authorTuanIds)
            if (tuanId) {
                if (!$.canHelp) break;
                console.log(`\n账号${$.UserName} 参加团 【${tuanId}】`), await JoinTuan(tuanId), await $.wait(1e3)
            }
}

function QueryActiveConfig() {
    return new Promise((resolve => {
        const options = taskTuanUrl("QueryActiveConfig", `activeId=${escape(tuanActiveId)}&tuanId=`, "_time,activeId,tuanId");
        $.get(options, (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                else if (safeGet(data))
                    ...
                    } else console.log(`QueryActiveConfig异常：${JSON.stringify(data)}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function QueryTuan(activeId, tuanId) {
    return new Promise((resolve => {
        const options = taskTuanUrl("QueryTuan", `activeId=${escape(activeId)}&tuanId=${escape(tuanId)}`, "_time,activeId,tuanId");
        $.get(options, (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (0 === (data = JSON.parse(data)).ret || console.log(`异常：${JSON.stringify(data)}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function CreateTuan() {
    return new Promise((resolve => {
        const options = taskTuanUrl("CreateTuan", `activeId=${escape(tuanActiveId)}&isOpenApp=1`, "_time,activeId,isOpenApp");
        $.get(options, (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (0 === (data = JSON.parse(data)).ret ? (console.log(`开团成功tuanId为\n${data.data.tuanId}`), $.tuanIds.push(data.data.tuanId)) : console.log(`开团异常：${JSON.stringify(data)}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function JoinTuan(tuanId, stk = "_time,activeId,tuanId") {
    return new Promise((resolve => {
        const options = taskTuanUrl("JoinTuan", `activeId=${escape(tuanActiveId)}&tuanId=${escape(tuanId)}`, "_time,activeId,tuanId");
        $.get(options, (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (0 === (data = JSON.parse(data)).ret ? console.log(`参团成功：${JSON.stringify(data)}\n`) : 10005 === data.ret || 10206 === data.ret ? (console.log(`参团失败：${JSON.stringify(data)}\n`), $.canHelp = !1) : console.log(`参团失败：${JSON.stringify(data)}\n`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function QueryAllTuan() {
    return new Promise((resolve => {
        const options = taskTuanUrl("QueryAllTuan", `activeId=${escape(tuanActiveId)}&pageNo=1&pageSize=10`, "_time,activeId,pageNo,pageSize");
        $.get(options, (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                else if (safeGet(data))
                    if (0 === (data = JSON.parse(data)).ret) {
                        ...
                        for (let item of tuanInfo)
                            if (item.tuanNum === item.realTuanNum) {
                                ...
                                for (let item2 of userInfo) item2.encryptPin === $.encryptPin && (item2.receiveElectric && item2.receiveElectric > 0 ? console.log(`${new Date(1e3*item2.joinTime).toLocaleString()}参加团主【${item2.nickName}】的奖励已经领取成功`) : (console.log(`开始领取${new Date(1e3*item2.joinTime).toLocaleString()}参加团主【${item2.nickName}】的奖励`), await tuanAward(item.tuanActiveId, item.tuanId, item.tuanLeader === $.encryptPin)))
                            } else console.log(`${new Date(1e3*item.beginTime).toLocaleString()}参加团主【${item.tuanLeader}】失败`)
                    } else console.log(`QueryAllTuan异常：${JSON.stringify(data)}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function tuanAward(activeId, tuanId, isTuanLeader = !0) {
    return new Promise((resolve => {
        const options = taskTuanUrl("Award", `activeId=${escape(activeId)}&tuanId=${escape(tuanId)}`, "_time,activeId,tuanId");
        $.get(options, (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (0 === (data = JSON.parse(data)).ret ? isTuanLeader ? (console.log(`开团奖励(团长)${data.data.electric}领取成功`), message += `【开团(团长)奖励】${data.data.electric}领取成功\n`, $.surplusOpenTuanNum > 0 && ($.log("开团奖励(团长)已领取，准备开团"), await CreateTuan())) : (console.log(`参团奖励${data.data.electric}领取成功`), message += `【参团奖励】${data.data.electric}领取成功\n`) : 10212 === data.ret ? (console.log(`${JSON.stringify(data)}`), isTuanLeader && $.surplusOpenTuanNum > 0 && ($.log("团奖励已领取，准备开团"), await CreateTuan())) : console.log(`异常：${JSON.stringify(data)}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function updateTuanIdsCDN(url) {
    return new Promise((async resolve => {
        ...
        $.get(options, ((err, resp, data) => {
            try {
                err || safeGet(data) && ($.tuanConfigs = data = JSON.parse(data))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
            }
        })), await $.wait(2e4), resolve()
    }))
}
async function exchangeProNotify() {
    ...
    $.shelvesList && $.shelvesList.length > 0 && console.log("\n  商品名     兑换状态");
    for (let shel of $.shelvesList) console.log(`${shel.name}    ${1===shel.exchangeStatus?"未兑换":2===shel.exchangeStatus?"已兑换":"兑换超时"}`), 1 === shel.exchangeStatus ? (exchangeEndTime = 1e3 * shel.exchangeEndTime, $.picture = shel.picture, exchangeEndHours = new Date(exchangeEndTime + 60 * (new Date).getTimezoneOffset() * 1e3 + 288e5).getHours(), $.exchangeEndTime = new Date(exchangeEndTime + 60 * (new Date).getTimezoneOffset() * 1e3 + 288e5).toLocaleString("zh", {
        hour12: !1
    }), nowHours = nowTimes.getHours()) : shel.exchangeStatus;
    if (exchangeEndTime && nowTimes < exchangeEndTime) {
        let flag = !0;
        if (exchangeEndTime - nowTimes.getTime() <= 144e5) {
            let expiredTime = parseFloat(((exchangeEndTime - nowTimes.getTime()) / 36e5).toFixed(1));
            $.msg($.name, "", `【京东账号${$.index}】${$.nickName}\n【生产商品】${$.productName}${expiredTime}小时后兑换超时\n【兑换截止时间】${$.exchangeEndTime}\n请速去京喜APP->首页->好物0元造进行兑换`, {
                "open-url": jxOpenUrl,
                "media-url": $.picture
            }), !$.isNode() || "1" != `${notifyLevel}` && "2" != `${notifyLevel}` && "3" != `${notifyLevel}` || (allMessage += `【京东账号${$.index}】${$.nickName}\n【生产商品】${$.productName}${expiredTime}小时后兑换超时\n【兑换截止时间】${$.exchangeEndTime}\n请速去京喜APP->首页->好物0元造进行兑换${$.index!==cookiesArr.length?"\n\n":""}`), flag = !1
        }
        nowHours % 2 == 0 && flag && ($.msg($.name, "", `【京东账号${$.index}】${$.nickName}\n【生产商品】${$.productName}已可兑换\n【兑换截止时间】${$.exchangeEndTime}\n请速去京喜APP->首页->好物0元造进行兑换`, {
            "open-url": jxOpenUrl,
            "media-url": $.picture
        }), !$.isNode() || "1" != `${notifyLevel}` && "2" != `${notifyLevel}` && "3" != `${notifyLevel}` || (allMessage += `【京东账号${$.index}】${$.nickName}\n【生产商品】${$.productName}已可兑换\n【兑换截止时间】${$.exchangeEndTime}\n请速去京喜APP->首页->好物0元造进行兑换${$.index!==cookiesArr.length?"\n\n":""}`))
    }
...
function requireConfig() {
    return new Promise((async resolve => {
        ...
        console.log(`共${cookiesArr.length}个京东账号\n`), $.shareCodesArr = [], $.isNode() ? Object.keys(shareCodes).forEach((item => {
            shareCodes[item] && $.shareCodesArr.push(shareCodes[item])
        })) : ($.getdata("jd_jxFactory") && ($.shareCodesArr = $.getdata("jd_jxFactory").split("\n").filter((item => "" !== item && null != item))), console.log(`\nBoxJs设置的${$.name}好友邀请码:${$.getdata("jd_jxFactory")}\n`)), console.log(`您提供了${$.shareCodesArr.length}个账号的${$.name}助力码\n`), resolve()
    }))
}
...
function taskTuanUrl(functionId, body = "", stk) {
    ...
    return url += `&h5st=${decrypt(Date.now(),stk||"","",url)}`, stk && (url += `&_stk=${encodeURIComponent(stk)}`), {
        url: url,
        headers: {
            Accept: "*/*",
            "Accept-Encoding": "gzip, deflate, br",
...
function taskurl(functionId, body = "", stk) {
    ...
    return url += `&h5st=${decrypt(Date.now(),stk,"",url)}`, stk && (url += `&_stk=${encodeURIComponent(stk)}`), {
        ...
        headers: {
            ...
            Accept: "*/*",
            Connection: "keep-alive",
            "User-Agent": "AssistFriend" === functionId ? "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.66 Safari/537.36" : "jdpingou",
            "Accept-Language": "zh-cn",
...
function newtasksysUrl(functionId, taskId, stk) {
    ...
    return taskId && (url += `&taskId=${taskId}`), stk && (url += `&_stk=${stk}`), url += `&h5st=${decrypt(Date.now(),stk,"",url)}`, {
        url: url,
        headers: {
            Cookie: cookie,
            Host: "m.jingxi.com",
...
$.tuanIds = [], $.appId = 10001, $.isNode() ? (Object.keys(jdCookieNode).forEach((item => {
    cookiesArr.push(jdCookieNode[item])
})), process.env.JD_DEBUG && "false" === process.env.JD_DEBUG && (console.log = () => {}), process.env.DREAMFACTORY_FORBID_ACCOUNT && process.env.DREAMFACTORY_FORBID_ACCOUNT.split("&").map(((item, index) => 0 === Number(item) ? cookiesArr = [] : cookiesArr.splice(Number(item) - 1 - index, 1)))) : cookiesArr = [$.getdata("CookieJD"), $.getdata("CookieJD2"), ...jsonParse($.getdata("CookiesJD") || "[]").map((item => item.cookie))].filter((item => !!item)), (async () => {
    if ($.CryptoJS = $.isNode() ? require("crypto-js") : CryptoJS, await requireConfig(), cookiesArr[0]) {
        await requestAlgo(), await getActiveId();
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                if (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.isLogin = !0, $.nickName = "", message = "", $.ele = 0, $.pickEle = 0, $.pickFriendEle = 0, $.friendList = [], $.canHelpFlag = !0, $.tuanNum = 0, await TotalBean(), console.log(`\n******开始【京东账号${$.index}】${$.nickName||$.UserName}*********\n`), !$.isLogin) {
            ...
            for (let i = 0; i < cookiesArr.length; i++)
                if (cookiesArr[i]) {
                    if (cookie = cookiesArr[i], $.isLogin = !0, $.canHelp = !0, $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), cookiesArr && cookiesArr.length >= ($.tuanNum || 5) && $.canHelp) {
                        console.log(`\n账号${$.UserName} 内部相互进团\n`);
                        for (let item of $.tuanIds) {
                            if (console.log(`\n${$.UserName} 去参加团 ${item}`), !$.canHelp) break;
                            await JoinTuan(item), await $.wait(1e3)
                        }
                    }
                    $.canHelp && await joinLeaderTuan()
