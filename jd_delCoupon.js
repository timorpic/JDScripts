...
let goodPageSize = $.getdata("jdUnsubscribePageSize") || 20,
    shopPageSize = $.getdata("jdUnsubscribeShopPageSize") || 20,
    stopGoods = $.getdata("jdUnsubscribeStopGoods") || "",
    stopShop = $.getdata("jdUnsubscribeStopShop") || "",
    delCount = 0,
    hasKeyword = 0;
...
function delCoupon(couponId, couponTitle) {
    return new Promise((resolve => {
        ...
        $.get(options, ((err, resp, data) => {
            try {
                0 === (data = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1])).retcode && (console.log(`删除优惠券---${couponTitle}----成功\n`), delCount++)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
            ...
            $.get(options, (async (err, resp, data) => {
                try {
                    ...
                    if ("6" === states[s]) {
                        ...
                        for (let i = 0; i < expire.length; i++) couponTitle = expire[i].couponTitle, couponId = escape(`${expire[i].couponid},1,0`), await delCoupon(couponId, couponTitle);
                        let used = data.coupon.used;
                        for (let i = 0; i < used.length; i++) couponTitle = used[i].couponTitle, couponId = escape(`${used[i].couponid},0,0`), await delCoupon(couponId, couponTitle)
                    } else if ("1" === states[s]) {
                        let useable = data.coupon.useable;
                        for (let i = 0; i < useable.length; i++) couponTitle = useable[i].limitStr, couponId = escape(`${useable[i].couponid},1,0`), isJDCoupon(couponTitle) ? ($.log(`跳过删除:${couponTitle}`), hasKeyword++) : await delCoupon(couponId, couponTitle)
                    }
                } catch (e) {
                    $.logErr(e, resp)
                } finally {
...
function isJDCoupon(title) {
    return title.indexOf("京东") > -1 || (title.indexOf("超市") > -1 || (title.indexOf("1元爆品") > -1 || (title.indexOf("京贴") > -1 || !(title.indexOf("国际") > -1) && (!(title.indexOf("旗舰店") > -1) && (title.indexOf("生鲜") > -1 || (title.indexOf("9.9减9") > -1 || title.indexOf("食品饮料") > -1))))))
}

function showMsg() {
    jdNotify && "false" !== jdNotify ? $.log(`\n【京东账号${$.index}】${$.nickName}\n【已删除优惠券】${delCount}张\n【跳过含关键词】${hasKeyword}张`) : $.msg($.name, "", `【京东账号${$.index}】${$.nickName}\n【已删除优惠券】${delCount}张\n【跳过含关键词】${hasKeyword}张`)
}
...
function Env(t, e) {
    ...
    cookiesArr[0] || $.msg("【京东账号一】删除优惠券失败", "【提示】请先获取京东账号一cookie\n直接使用NobyDa的京东签到获取", "https://bean.m.jd.com/bean/signIndex.action", {
