...
async function price() {
    let num = 0;
    do {
        await jstoken(), $.token && await siteppM_skuOnceApply(), num++
    } while (num < 3 && !$.token);
    await showMsg()
}
async function siteppM_skuOnceApply() {
    ...
    return new Promise((async resolve => {
        $.post(taskUrl("siteppM_skuOnceApply", body), (async (err, resp, data) => {
            try {
                err ? (console.log(JSON.stringify(err)), console.log(`${$.name} siteppM_skuOnceApply API请求失败，请检查网路重试`)) : safeGet(data) && ((data = JSON.parse(data)).flag ? await siteppM_appliedSuccAmount() : (console.log(`保价失败：${data.responseMessage}`), message += `保价失败：${data.responseMessage}\n`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function siteppM_appliedSuccAmount() {
    ...
    return new Promise((resolve => {
        $.post(taskUrl("siteppM_appliedSuccAmount", body), ((err, resp, data) => {
            try {
                err ? (console.log(JSON.stringify(err)), console.log(`${$.name} siteppM_appliedSuccAmount API请求失败，请检查网路重试`)) : safeGet(data) && ((data = JSON.parse(data)).flag ? (console.log(`保价成功：返还${data.succAmount}元`), message += `保价成功：返还${data.succAmount}元\n`) : console.log("保价失败：没有可保价的订单"))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
async function jstoken() {
    ...
    let dom = new JSDOM('<body><script src="https://js-nocaptcha.jd.com/statics/js/main.min.js"><\/script></body>', {
...
function showMsg() {
    return new Promise((resolve => {
        message && (allMessage += `【京东账号${$.index}】${$.nickName||$.UserName}\n${message}${$.index,cookiesArr.length,"\n\n"}`), $.msg($.name, "", `【京东账号${$.index}】${$.nickName}\n${message}`), resolve()
    }))
}
...
function Env(t, e) {
    ...
    if (cookiesArr[0]) {
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                if (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.isLogin = !0, $.nickName = "", $.token = "", message = "", await TotalBean(), console.log(`\n******开始【京东账号${$.index}】${$.nickName||$.UserName}*********\n`), !$.isLogin) {
                    $.msg($.name, "【提示】cookie已失效", `京东账号${$.index} ${$.nickName||$.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {
                        "open-url": "https://bean.m.jd.com/bean/signIndex.action"
                    }), $.isNode() && await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
                    continue
                }
                await price(), await $.wait(2e3)
