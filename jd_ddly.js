...
function browseAward(id, index, type) {
    return new Promise((async resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data)).result ? console.log("领取奖励成功,获得💧" + data.result.waterEnergy) : console.log(JSON.stringify(data))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function browse(id) {
    return new Promise((async resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data), console.log(`浏览 ${id}  : ${data.success}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function parkInit() {
    return new Promise((async resolve => {
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data)).buildings ? $.taskList = data.buildings.filter((x => x.topResource.task)) : console.log("获取任务列表失败,你不会是黑鬼吧")
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
    ...
    if (cookiesArr[0]) {
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookie = cookiesArr[i], cookie) {
                if ($.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.isLogin = !0, $.nickName = "", $.beans = 0, $.taskList = [], message = "", console.log(`\n******开始【京东账号${$.index}】${$.nickName||$.UserName}*********\n`), !$.isLogin) {
                    $.msg($.name, "【提示】cookie已失效", `京东账号${$.index} ${$.nickName||$.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {
                        "open-url": "https://bean.m.jd.com/bean/signIndex.action"
                    }), $.isNode() && await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
                    continue
                }
                for (task of (await parkInit(), $.taskList))
                    if (3 == task.topResource.task.status) console.log(`任务 ${task.topResource.title} 已完成`);
                    else {
                        console.log("去浏览：" + task.topResource.title);
                        let index = task.name.match(/\d+/)[0] - 1;
                        console.log(task.topResource.task.advertId, index, task.type), await browse(task.topResource.task.advertId), await $.wait(1e3), await browseAward(task.topResource.task.advertId, index, task.type)
                    }
            }
