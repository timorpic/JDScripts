...
function unsubscribeCartsFun() {
    return new Promise((resolve => {
        ...
        $.post(options, ((err, resp, data) => {
            try {
                "0" == (data = JSON.parse(data)).errId && console.log("清空购物车成功")
            } catch (e) {
                console.log("清空购物车出错"), $.logErr(e, resp)
            } finally {
                resolve(data)
            }
        }))
...
function getStr(text, start, end) {
    var str = text,
        aPos = str.indexOf(start);
    if (aPos < 0) return null;
    var bPos = str.indexOf(end, aPos + start.length);
    return bPos < 0 ? null : str.substr(aPos + start.length, text.length - (aPos + start.length) - (text.length - bPos))
}

function getCarts() {
    return $.shopsTotalNum = 0, new Promise((resolve => {
        const option = {
        ...
        $.get(option, ((err, resp, data) => {
            try {
                if (data = JSON.parse(getStr(data, "window.cartData =", "window._PFM_TIMING")), $.cartsTotalNum = 0, "0" === data.errId) {
                    let itemId, sKuId, index, temp;
                    $.traceId = data.traceId, $.areaId = data.areaId, $.commlist = "";
                    for (let i = 0; i < data.cart.venderCart.length; i++) {
                        ...
                        for (let s = 0; s < vender.sortedItems.length; s++) {
                            ...
                            for (let m = 0; m < sorted.polyItem.products.length; m++) {
                                ...
                                itemId == products.mainSku.id ? (sKuId = "", index = "1") : (sKuId = itemId, index = "4" == sorted.polyType ? "13" : "11"), temp = [products.mainSku.id, , "1", products.mainSku.id, index, sKuId, "0", "skuUuid:" + products.skuUuid + "@@useUuid:" + products.useUuid].join(","), $.commlist.length > 0 && ($.commlist += "$"), $.commlist += temp, $.cartsTotalNum += 1
                            }
                        }
                    }
                    $.commlist.length > 0 && ($.commlist = encodeURIComponent($.commlist)), console.log(`当前购物车商品数：${$.cartsTotalNum}个\n`)
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
$.isNode() ? (Object.keys(jdCookieNode).forEach((item => {
...
})), process.env.JD_DEBUG && "false" === process.env.JD_DEBUG && (console.log = () => {}), users = process.env.CleanUsers ? process.env.CleanUsers : "") : cookiesArr = [$.getdata("CookieJD"), $.getdata("CookieJD2"), ...jsonParse($.getdata("CookiesJD") || "[]").map((item => item.cookie))].filter((item => !!item)), (async () => {
    cookiesArr[0] || $.msg("【京东账号一】清空购物车失败", "【提示】请先获取京东账号一cookie\n直接使用NobyDa的京东签到获取", "https://bean.m.jd.com/bean/signIndex.action", {
    ...
    for (let i = 0; i < cookiesArr.length; i++)
        if (cookiesArr[i]) {
            ...
            if (allMessage += `京东账号${$.index} - ${$.nickName}\n`, await getCarts(), allMessage += `购物车商品数：${$.cartsTotalNum}\n`, $.cartsTotalNum > 0) {
                for (let i = 0; i < 3 && (await unsubscribeCartsFun(), await getCarts(), 0 != $.cartsTotalNum); i++) await sleep(3e3);
                $.cartsTotalNum > 0 ? allMessage += "清空结果：❌\n" : allMessage += "清空结果：✅\n"
            }
            allMessage += "\n"
        } allMessage && (allMessage = allMessage.substring(0, allMessage.length - 1), !$.isNode() || process.env.CASH_NOTIFY_CONTROL && "false" !== process.env.CASH_NOTIFY_CONTROL || await notify.sendNotify($.name, allMessage))
})().catch((e => {
    $.log("", `❌ ${$.name}, 失败! 原因: ${e}!`, "")
})).finally((() => {
    $.done()
