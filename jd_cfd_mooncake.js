...
const $ = new Env("京喜财富岛合成月饼_Timorpic"),
...
async function cfd() {
    try {
        ...
        await $.wait(2e3), await composePearlState(4), await $.wait(2e3), await composePearlState(2);
        let count = $.isNode() ? process.env.JD_CFD_RUNNUM ? 1 * process.env.JD_CFD_RUNNUM : Math.floor(2 * Math.random()) + 3 : $.getdata("JD_CFD_RUNNUM") ? 1 * $.getdata("JD_CFD_RUNNUM") : Math.floor(2 * Math.random()) + 3;
        console.log("\n合成月饼"), console.log(`合成月饼运行次数为：${count}\n`);
        let num = 0;
        do {
            await $.wait(2e3), await composePearlState(3), num++
...
async function composePearlState(type) {
    return new Promise((async resolve => {
        $.get(taskUrl("user/ComposePearlState", `__t=${Date.now()}&dwGetType=0`), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} ComposePearlState API请求失败，请检查网路重试`);
                else switch (type) {
                    case 1:
                        data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]);
                        break;
                    case 2:
                        if (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]), console.log("领助力奖励"), 0 === data.iRet) {
                            let helpNum = [];
                            for (let key of Object.keys(data.helpInfo.HelpList)) {
                                let vo = data.helpInfo.HelpList[key];
                                1 !== vo.dwStatus && 1 === vo.dwIsHasAward && 1 === vo.dwIsHelp && helpNum.push(vo.dwId)
                            }
                            if (0 !== helpNum.length)
                                for (let j = 0; j < helpNum.length; j++) await pearlHelpDraw(data.ddwSeasonStartTm, helpNum[j]), await $.wait(2e3), data = await composePearlState(1);
                            else console.log("暂无可领助力奖励")
                        }
                        break;
                    case 3:
                        if (0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet)
                            if (console.log(`当前已合成${data.dwCurProgress}颗月饼，总计获得${data.ddwVirHb/100}元红包`), data.strDT) {
                                let beacon = data.PearlList[0];
                                data.PearlList.shift();
                                let beaconType = beacon.type,
                                    num = Math.ceil(12 * Math.random() + 12);
                                console.log(`合成月饼：模拟操作${num}次`);
                                for (let v = 0; v < num; v++)
                                    if (console.log(`模拟操作进度：${v+1}/${num}`), await $.wait(2e3), await realTmReport(data.strMyShareId), beacon.rbf) {
                                        let size = 1;
                                    ...
                                    res = await composePearlAddProcess(data.strDT, strLT);
                                0 === res.iRet ? (console.log(`\n合成月饼成功：获得${res.ddwAwardHb/100}元红包\n`), 0 === res.ddwAwardHb && ($.stop = !0, console.log("合成月饼没有奖励，停止运行\n"))) : console.log(`\n合成月饼失败：${res.sErrMsg}\n`)
                            } else console.log("今日已完成\n");
                        break;
                    case 4:
                        if (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]), console.log("每日抽奖"), 0 === data.iRet)
                            if (0 === data.dayDrawInfo.dwIsDraw) {
                                let strToken = (await getPearlDailyReward()).strToken;
                                await $.wait(2e3), await pearlDailyDraw(data.ddwSeasonStartTm, strToken)
                            } else console.log("无抽奖次数\n")
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
function realTmReport(strMyShareId) {
    return new Promise((resolve => {
        $.get(taskUrl("user/RealTmReport", `__t=${Date.now()}&dwIdentityType=0&strBussKey=composegame&strMyShareId=${strMyShareId}&ddwCount=10`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} RealTmReport API请求失败，请检查网路重试`)) : data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function composePearlAddProcess(strDT, strLT) {
    return new Promise((resolve => {
        $.get(taskUrl("user/ComposePearlAddProcess", `strBT=${strDT}&strLT=${strLT}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} ComposePearlAddProcess API请求失败，请检查网路重试`)) : data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getPearlDailyReward() {
    return new Promise((resolve => {
        $.get(taskUrl("user/GetPearlDailyReward", `__t=${Date.now()}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} GetPearlDailyReward API请求失败，请检查网路重试`)) : data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function pearlDailyDraw(ddwSeasonStartTm, strToken) {
    return new Promise((resolve => {
        $.get(taskUrl("user/PearlDailyDraw", `__t=${Date.now()}&ddwSeaonStart=${ddwSeasonStartTm}&strToken=${strToken}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} PearlDailyDraw API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet ? console.log(`抽奖成功：获得${data.strPrizeName||JSON.stringify(data)}\n`) : console.log(`抽奖失败：${data.sErrMsg}\n`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function composePearlAward(strDT, type, size) {
    return new Promise((resolve => {
        $.get(taskUrl("user/ComposePearlAward", `__t=${Date.now()}&type=${type}&size=${size}&strBT=${strDT}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} ComposePearlAward API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet ? console.log(`模拟操作中奖：获得${data.ddwAwardHb/100}元红包，总计获得${data.ddwVirHb/100}元红包`) : console.log(`模拟操作未中奖：${data.sErrMsg}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function pearlHelpDraw(ddwSeasonStartTm, dwUserId) {
    return new Promise((resolve => {
        $.get(taskUrl("user/PearlHelpDraw", `__t=${Date.now()}&ddwSeaonStart=${ddwSeasonStartTm}&dwUserId=${dwUserId}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} PearlHelpDraw API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet ? console.log(`领取助力奖励成功：获得${data.StagePrizeInfo.ddwAwardHb/100}元红包，总计获得${data.StagePrizeInfo.ddwVirHb/100}元红包`) : console.log(`领取助力奖励失败：${data.sErrMsg}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
$.appId = 10032, $.isNode() ? (Object.keys(jdCookieNode).forEach((item => {
    ...
    res || ($.http.get({
        url: "https://purge.jsdelivr.net/gh/asd920/updateTeam@main/shareCodes/cfd.json"
    }).then((resp => {})).catch((e => console.log("刷新CDN异常", e))), await $.wait(1e3), res = await getAuthorShareCode("https://cdn.jsdelivr.net/gh/asd920/updateTeam@main/shareCodes/cfd.json")), $.strMyShareIds = [...res && res.shareId || []], await shareCodesFormat();
