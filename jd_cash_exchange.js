...
function exchange() {
    return new Promise((resolve => {
        $.get(taskUrl("v1/user/exchange/bean/check"), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试\n`)) : safeGet(data) && (data = JSON.parse(data)).status && console.log(`兑换结果：${data.data.reason}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function Env(t, e) {
    ...
    if (cookiesArr[0]) {
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.isLogin = !0, $.nickName = "", message = "", console.log(`=====京东账号${$.index} ${$.UserName}=====`);
                for (let j = 0; j < 10; ++j) await exchange()
            }
