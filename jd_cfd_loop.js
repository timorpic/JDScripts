...
const $ = new Env("京喜财富岛热气球_Timorpic"),
            ...
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} querystorageroom API请求失败，请检查网路重试`);
                    ...
                    if (0 !== bags.length) {
                        ...
                        await $.wait(3e3), await sellgoods(`strTypeCnt=${strTypeCnt}&dwSceneId=1`)
...
async function queryshell() {
    return new Promise((async resolve => {
        $.get(taskUrl("story/queryshell"), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} queryshell API请求失败，请检查网路重试`);
                else {
                    data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]);
                    for (let key of Object.keys(data.Data.NormShell)) {
                        let vo = data.Data.NormShell[key];
                        for (let j = 0; j < vo.dwNum; j++) await $.wait(3e3), await pickshell(`dwType=${vo.dwType}`)
                    }
                    console.log("")
                }
            } catch (e) {
...
async function pickshell(body) {
    return new Promise((async resolve => {
        $.get(taskUrl("story/pickshell", body), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} pickshell API请求失败，请检查网路重试`);
                else {
                    let dwName;
                    switch ((data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).Data.strFirstDesc) {
                        case "亲爱的岛主~♥七夕快乐鸭♥":
                            dwName = "爱心珍珠";
                            break;
                        case "捡到珍珠了，看起来很贵的样子":
                    ...
                    0 === data.iRet ? console.log(`捡贝壳成功：捡到了${dwName}`) : 5403 === data.iRet || "这种小贝壳背包放不下啦，先去卖掉一些吧~" === data.sErrMsg ? (console.log(`捡贝壳失败：${data.sErrMsg}`), await $.wait(3e3), await querystorageroom()) : console.log(`捡贝壳失败：${data.sErrMsg}`)
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
$.appId = 10032, $.isNode() ? (Object.keys(jdCookieNode).forEach((item => {
    ...
    let count = 0;
    $.CryptoJS = $.isNode() ? require("crypto-js") : CryptoJS, await requestAlgo(), await $.wait(1e3), console.log("\n");
    do {
        count++, console.log(`============开始第${count}次挂机=============`);
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.nickName = "", $.isLogin = !0, console.log(`\n******开始【京东账号${$.index}】${$.nickName||$.UserName}*********\n`), $.info = {}, UA = UAInfo[$.UserName] ? UAInfo[$.UserName] : `jdpingou;iPhone;4.13.0;14.4.2;${randomString(40)};network/wifi;model/iPhone10,2;appBuild/100609;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${98*Math.random+1};pap/JA2019_3111789;brand/apple;supportJDSHWK/1;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148`, token = await getJxToken(), await cfd();
                let time = process.env.CFD_LOOP_SLEEPTIME ? 1 * process.env.CFD_LOOP_SLEEPTIME > 1e3 ? process.env.CFD_LOOP_SLEEPTIME : 1e3 * process.env.CFD_LOOP_SLEEPTIME : 5e3;
                await $.wait(time), UAInfo[$.UserName] = UA
            }
    } while (count < 25)
