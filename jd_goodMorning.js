const $ = Env("早起福利_Timorpic"),
    ua = `jdltapp;iPhone;3.1.0;${Math.ceil(4*Math.random()+10)}.${Math.ceil(4*Math.random())};${randomString(40)}`;
let cookiesArr = [],
    cookie = "";
...
function goodMorning() {
    return new Promise((resolve => {
        $.get({
            ...
            try {
                (data = JSON.parse(data)).data && console.log(data.data.bizMsg), data.errorMessage && console.log(data.errorMessage)
            } catch (e) {
                $.logErr("Error: ", e, resp)
            } finally {
                resolve(data)
...
function Env(t, e) {
    ...
    if (await requireConfig(), cookiesArr[0]) {
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                if (cookie = cookiesArr[i], $.cookie = cookie, $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.isLogin = !0, $.nickName = "", message = "", await TotalBean(), console.log(`\n开始【京东账号${$.index}】${$.nickName||$.UserName}\n`), !$.isLogin) {
