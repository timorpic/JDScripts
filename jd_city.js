const $ = new Env("城城领现金_Timorpic"),
    notify = $.isNode() ? require("./sendNotify") : "",
    jdCookieNode = $.isNode() ? require("./jdCookie.js") : "";
let message, exchangeFlag = $.getdata("jdJxdExchange") || !1,
    cookiesArr = [],
    cookie = "";
...
let inviteCodes = ["H4znzO6lRQ2-d4WcEdA7mk5nlgZkND7qRmRL3-NFhevZimfA@lEljyd4WYRtU2168p_c1wQZP9B2nqV5nnHy2lN9xRAA@HY3iyO2gRg6ld4r1V5h_mn1pTEhcfhc3aB-4-6u7Em0IqA@Hobkyu6jSAqgY4qaHtY20stBFY7aTn8m6L1U1o4ODmVajt78T9qH", "H4znzO6lRQ2-d4WcEdA7mk5nlgZkND7qRmRL3-NFhevZimfA@lEljyd4WYRtU2168p_c1wQZP9B2nqV5nnHy2lN9xRAA@HY3iyO2gRg6ld4r1V5h_mn1pTEhcfhc3aB-4-6u7Em0IqA@Hobkyu6jSAqgY4qaHtY20stBFY7aTn8m6L1U1o4ODmVajt78T9qH", "H4znzO6lRQ2-d4WcEdA7mk5nlgZkND7qRmRL3-NFhevZimfA@lEljyd4WYRtU2168p_c1wQZP9B2nqV5nnHy2lN9xRAA@HY3iyO2gRg6ld4r1V5h_mn1pTEhcfhc3aB-4-6u7Em0IqA@Hobkyu6jSAqgY4qaHtY20stBFY7aTn8m6L1U1o4ODmVajt78T9qH"];
...
function getInfo(inviteId, flag = !1) {
    let body = {
        lbsCity: "19",
        realLbsCity: "1601",
        inviteId: inviteId,
    ...
    return new Promise((resolve => {
        $.post(taskPostUrl("city_getHomeData", body), (async (err, resp, data) => {
            try {
                ...
                else if (safeGet(data) && ((data = JSON.parse(data)).data && !data.data.result.userActBaseInfo.inviteId ? console.log("账号已黑，看不到邀请码") : flag && console.log(`\n【京东账号${$.index}（${$.UserName}）的${$.name}好友互助码】${data.data&&data.data.result.userActBaseInfo.inviteId}\n`), data.data && 0 === data.data.bizCode))
                    for (let vo of data.data.result && data.data.result.mainInfos || []) vo && 0 === vo.remaingAssistNum && "1" === vo.status && (console.log(vo.roundNum), await receiveCash(vo.roundNum), await $.wait(2e3))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function receiveCash(roundNum) {
    ...
    return new Promise((resolve => {
        $.post(taskPostUrl("city_receiveCash", body), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (console.log(`领红包结果${data}`), 0 === (data = JSON.parse(data)).data.bizCode && console.log(`获得 ${data.data.result.currentTimeCash} 元，共计 ${data.data.result.totalCash} 元`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function city_lotteryAward() {
    ...
    return new Promise((resolve => {
        $.post(taskPostUrl("city_lotteryAward", body), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                else if (safeGet(data) && (console.log(`抽奖结果：${data}`), 0 === (data = JSON.parse(data)).data.bizCode)) {
...
function readShareCode() {
    return console.log("开始"), new Promise((async resolve => {
        $.get({
            url: "http://share.turinglabs.net/api/v3/city/query/1/",
            timeout: 1e4
        }, ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : data && (data = JSON.parse(data))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function requireConfig() {
    return new Promise((resolve => {
        ...
        let shareCodes = [];
        $.isNode() && (process.env.JD_CITY_EXCHANGE && (exchangeFlag = process.env.JD_CITY_EXCHANGE || exchangeFlag), process.env.CITY_SHARECODES && (shareCodes = process.env.CITY_SHARECODES.indexOf("\n") > -1 ? process.env.CITY_SHARECODES.split("\n") : process.env.CITY_SHARECODES.split("&"))), console.log(`共${cookiesArr.length}个京东账号\n`), $.shareCodesArr = [], $.isNode() && Object.keys(shareCodes).forEach((item => {
...
function Env(t, e) {
    ...
    if (cookiesArr[0]) {
        await requireConfig(), exchangeFlag ? console.log("脚本自动抽奖") : console.log("脚本不会自动抽奖，建议活动快结束开启，默认关闭");
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                ...
                await shareCodesFormat(), await getInfo("", !0);
                for (let i = 0; i < $.newShareCodes.length; ++i) {
                    console.log(`开始助力 【${$.newShareCodes[i]}】`);
                    let res = await getInfo($.newShareCodes[i]);
                    if (res && 0 === res.data.bizCode) {
                        if (res.data.result.toasts && res.data.result.toasts[0] && "3" === res.data.result.toasts[0].status) {
                            console.log("助力次数已耗尽，跳出");
                            break
                        }
                        res.data.result.toasts && res.data.result.toasts[0] && console.log(`助力 【${$.newShareCodes[i]}】:${res.data.result.toasts[0].msg}`)
                    }
                    if (res && res.status && "3" === res.status || res && res.data && -11 === res.data.bizCode) break
                }
                if (await getInviteInfo(), exchangeFlag) {
                    const res = await city_lotteryAward();
                    if (res && res > 0)
                        for (let i = 0; i < new Array(res).fill("").length; i++) await $.wait(1e3), await city_lotteryAward()
                }
                await $.wait(1e3)
            }
