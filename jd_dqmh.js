const $ = new Env("电器盲盒抽京豆_Timorpic"),
...
var myDate = new Date,
    ...
    day = myDate.getDay();
let message, secretp, cookiesArr = [],
    isLoginInfo = {};
...
function getTaskDetail() {
    return new Promise((async resolve => {
        const options = taskJDZZUrl("healthyDay_getHomeData", {
            taskToken: "",
            appId: "1FFdSxqw",
            channelId: 6
        });
                            ...
                            for (let vo of data.data.result.taskVos) {
                                ...
                                if (21 !== vo.taskType || "card" == process.env.FS_LEVEL) {
                                    ...
                                    for (let i = vo.times, j = 0; i < vo.maxTimes; ++i, ++j) {
                                        $.taskBo = !0;
                                        let item = tmps[j];
                                        console.log(`去做${vo.taskName}任务，${i+1}/${vo.maxTimes}`), $.duration = vo.waitDuration + 1, $.duration < 9 && ($.duration = 9), await harmony_collectScore(vo.taskId, item.taskToken, 1), 21 === vo.taskType && $.index <= 10 && await join(item.vendorIds), await $.wait(1e3 * ($.duration + Math.random())), await harmony_collectScore(vo.taskId, item.taskToken, 0), await $.wait(3e3)
                                    }
                                } else console.log("不入会,FS_LEVEL=card入会")
                            }
                            for (; $.userScore >= 500;) $.duration < 9 && ($.duration = 9), await interact_template_getLotteryResult(), await $.wait(1e3 * ($.duration + Math.random()))
                        }
...
function harmony_collectScore(taskId, itemToken, actionType) {
    return new Promise((resolve => {
        var options = taskPostUrl("harmony_collectScore", {
            taskId: taskId,
            taskToken: itemToken,
        ...
        $.post(options, (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data)) && 0 === data.data.bizCode ? (data = data.data).result.score ? ($.userScore += data.result.score, console.log(`任务完成成功，获得${data.result.score}电力值`)) : data.result.taskToken ? console.log(`任务请求成功，等待${$.duration}秒`) : console.log("任务请求结果未知") : console.log(data.data.bizMsg)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function interact_template_getLotteryResult() {
    return new Promise((resolve => {
        var options = taskPostUrl("interact_template_getLotteryResult", {
            appId: "1FFdSxqw"
        });
        $.post(options, (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data)) && 0 === data.data.bizCode ? (data = data.data, $.userScore = data.result.userScore || $.userScore - 500, data.result.userAwardsCacheDto && data.result.userAwardsCacheDto.jBeanAwardVo ? ($.jdCount += parseInt(data.result.userAwardsCacheDto.jBeanAwardVo.quantity), console.log(`获得${data.result.userAwardsCacheDto.jBeanAwardVo.quantity}${data.result.userAwardsCacheDto.jBeanAwardVo.prizeName}，等待${$.duration}秒`)) : console.log(`什么也没抽到，等待${$.duration}秒`)) : console.log(data.data.bizMsg)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function getshopactivityId(venderId) {
    return new Promise((resolve => {
        $.get(shopactivityId(`${venderId}`), (async (err, resp, data) => {
            try {
                1 == (data = JSON.parse(data)).success && ($.shopactivityId = data.result.interestsRuleList && data.result.interestsRuleList[0] && data.result.interestsRuleList[0].interestsInfo && data.result.interestsRuleList[0].interestsInfo.activityId || "")
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function join(venderId) {
    return new Promise((async resolve => {
        $.shopactivityId = "", await $.wait(1e3), await getshopactivityId(venderId), $.get(ruhui(`${venderId}`), (async (err, resp, data) => {
            try {
                if (res = $.toObj(data), "object" == typeof res)
                    if (!0 === res.success) {
                        if (console.log(res.message), res.result && res.result.giftInfo)
                            for (let i of res.result.giftInfo.giftList) console.log(`入会获得:${i.discountString}${i.prizeName}${i.secondLineDesc}`)
                    } else "object" == typeof res && res.message ? console.log(`${res.message||""}`) : console.log(data);
...
function ruhui(functionId) {
    let activityId = "";
    return $.shopactivityId && (activityId = `,"activityId":${$.shopactivityId}`), {
...
function transform(str) {
    var REQUEST = new Object,
        aParams = str.slice(str.indexOf("?"), str.length - 1).substr(1).split("&");
...
$.isNode() ? (Object.keys(jdCookieNode).forEach((item => {
    ...
    })), process.env.JD_DEBUG && "false" === process.env.JD_DEBUG && (console.log = () => {})) : cookiesArr = [$.getdata("CookieJD"), $.getdata("CookieJD2"), ...$.toObj($.getdata("CookiesJD") || "[]").map((item => item.cookie))].filter((item => !!item)), $.UA = "Mozilla/5.0 (iPhone; CPU iPhone OS 14_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.15(0x18000f2c) NetType/WIFI Language/zh_CN", $.shareCodes = [], $.shareCodesArr = {}, $.newShareCodes = [], $.jdCount = 0, $.userScore = 0, (async () => {
        if (!(11 == month && day < 21)) {
            var msg = "活动已过期，请删除脚本";
            return console.info(msg), void await notify.sendNotify(`${$.name}活动已过期`, msg)
        }
        if (cookiesArr[0]) {
            $.activityId = "", $.totalScore = 0, $.taskBo = !1, $.homeMainInfo = {}, $.inviteIdCodesArr = {}, console.log("开始获取活动信息");
            msg = "";
            for (let i = 0; i < cookiesArr.length; i++) $.index = i + 1, $.cookie = cookiesArr[i], $.UserName = decodeURIComponent($.cookie.match(/pt_pin=([^; ]+)(?=;?)/) && $.cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.pre_session = decodeURIComponent($.cookie.match(/pre_session=([^; ]+)(?=;?)/) && $.cookie.match(/pre_session=([^; ]+)(?=;?)/)[1]), $.pre_session && ($.pre_session = $.pre_session.split("|")[0]), "null" == $.pre_session && ($.pre_session = randomString(40)), $.isLogin = !0, $.nickName = "", !1 === isLoginInfo[$.UserName] || isLoginInfo[$.UserName] || (await TotalBean(), isLoginInfo[$.UserName] = $.isLogin), isLoginInfo[$.UserName] ? (console.log(`\n******开始【京东账号${$.index}】${$.nickName||$.UserName}*********\n`), msg += `\n******开始【京东账号${$.index}】${$.nickName||$.UserName}*********\n`, $.jdCount = 0, await getTaskDetail(), msg += `\n本次运行获得${$.jdCount}个京豆\n`, console.log(`\n本次运行获得${$.jdCount}个京豆\n`)) : $.isNode() && await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
