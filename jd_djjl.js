...
async function main() {
    ...
    for (let i = 0; i < tasks.length; i++) {
        ...
        switch (i) {
            case 0:
                "1" === task_vos.status && await do_task(task_vos.simple_record_info_vo.task_token, task_vos.task_id, task_vos.task_type);
                continue;
            case 1:
                await getShareCode(task_vos.assist_task_detail_vo.task_token), await $.wait(1e3), getAssist(), await $.wait(1e3), console.log(`第${$.index}个账号${$.UserName}去助力第${Math.floor(($.index-1)/6)+1}个账号。`), doAssist(), await $.wait(1e3);
                continue;
            case 2:
        ...
        let taskId = task_vos.task_id,
            taskType = task_vos.task_type;
        for (let t of product_info_vos)
            if ("1" === t.status) {
                console.log(`开始任务：${task_vos.task_name}`);
                await do_task(t.task_token, taskId, taskType);
                await $.wait(1e3)
            }
    }
}
...
function getShareCode(token) {
    return new Promise((resolve => {
        $.get({
            ...
            try {
                data = $.toObj(data), shareCodes.push({
                    tid: token,
...
function doAssist() {
    return new Promise((resolve => {
        $.post({
            ...
            try {
                "0" === (data = $.toObj(data)).status && console.log("助力成功")
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function getAssist() {
    return new Promise((resolve => {
        $.get({
            ...
            try {
                data = $.toObj(data), console.log(`今日共收到${data.body.length}个助力`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function getIsvToken() {
    ...
    return new Promise((resolve => {
        $.post(config, (async (err, resp, data) => {
            try {
                err ? (console.log(`${$.name} API请求失败，请检查网路重试`), console.log(`${JSON.stringify(err)}`)) : (data = JSON.parse(data), $.isvToken = data.tokenKey, console.log(`isvToken:${$.isvToken}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getIsvToken2() {
    ...
    return new Promise((resolve => {
        $.post(config, (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data), $.token2 = data.token, console.log(`token2:${$.token2}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getToken() {
    ...
    return new Promise((resolve => {
        $.post(config, (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : (data = JSON.parse(data), $.token = data.body.access_token, console.log($.token))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function do_task(token, id, type) {
    return new Promise((resolve => {
        $.post({
            ...
            try {
                if (!err)
                    if ("0" === (data = JSON.parse(data)).status) {
                        let result = data.body.result;
                        console.log(`任务成功：本次获得 ${result.acquired_score}，账户总额 ${result.user_score}`), resolve(200)
                    } else console.log("任务失败！"), resolve(502)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function makeShareCode() {
    return new Promise((resolve => {
        $.post({
            ...
            try {
                err || (data = JSON.parse(data), resolve(data.data))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function get_produce_coins() {
    return new Promise((resolve => {
        $.post({
            ...
            try {
                if (!err)
                    if (data = JSON.parse(data), console.log("收币：", data), "0" === data.status) {
                        let coins = parseInt(data.body.coins);
                        console.log(`收币成功：获得 ${coins}`)
                    } else console.log("收币失败！"), resolve(500)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(200)
...
function Env(t, e) {
    ...
    if (await requireConfig(), cookiesArr[0]) {
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                if (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.isLogin = !0, $.nickName = "", $.shareCode = await makeShareCode(), await TotalBean(), console.log(`\n开始【京东账号${$.index}】${$.nickName||$.UserName}\n`), !$.isLogin) {
                    $.msg($.name, "【提示】cookie已失效", `京东账号${$.index} ${$.nickName||$.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {
                        "open-url": "https://bean.m.jd.com/bean/signIndex.action"
                    }), $.isNode() && await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
                    continue
                }
                if (message = "", subTitle = "", option = {}, await getIsvToken(), await getIsvToken2(), await getToken(), 200 !== await get_produce_coins()) continue;
                await $.wait(1e3), await main()
