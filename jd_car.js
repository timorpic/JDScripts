...
async function jdCar() {
    await check(), await sign(), await $.wait(1e3), await mission(), await $.wait(1e3), await getPoint()
}
...
function check() {
    return new Promise((resolve => {
        $.get(taskUrl("v1/user/exchange/bean/check"), ((err, resp, data) => {
            try {
                err ? (data = JSON.parse(resp.body), console.log(`${data.error.msg}`), message += `签到失败，${data.error.msg}\n`) : safeGet(data) && (data = JSON.parse(data), console.log(`兑换结果：${JSON.stringify(data)}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function sign() {
    return new Promise((resolve => {
        $.post(taskUrl("v1/user/sign"), ((err, resp, data) => {
            try {
                err ? (data = JSON.parse(resp.body), console.log(`${data.error.msg}`), message += `签到失败，${data.error.msg}\n`) : safeGet(data) && (data = JSON.parse(data)).status && (console.log(`签到成功，获得${data.data.point}，已签到${data.data.signDays}天`), message += `签到成功，获得${data.data.point}，已签到${data.data.signDays}天\n`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
    ...
    return new Promise((resolve => {
        $.get(taskUrl("v1/user/mission"), (async (err, resp, data) => {
            try {
                ...
                else if (safeGet(data) && (data = JSON.parse(data)).status) {
                    let missions = data.data.missionList;
                    for (let i = 0; i < missions.length; ++i) {
                        const mission = missions[i];
                        0 !== mission.missionStatus || 1 !== mission.missionType && 5 !== mission.missionType || (console.log(`去做任务：${mission.missionName}`), await doMission(mission.missionId), await $.wait(1e3))
                    }
                }
            } catch (e) {
                $.logErr(e, resp)
...
function doMission(missionId) {
    return new Promise((resolve => {
        $.post(taskPostUrl("v1/game/mission", {
            ...
            try {
                err ? (data = JSON.parse(resp.body), console.log(`${data.error.msg}`)) : safeGet(data) && (data = JSON.parse(data)).status && (console.log("任务领取成功"), await receiveMission(missionId))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function receiveMission(missionId) {
    return new Promise((resolve => {
        $.post(taskPostUrl("v1/user/mission/receive", {
            ...
            try {
                err ? (data = JSON.parse(resp.body), console.log(`${data.error.msg}`)) : safeGet(data) && (data = JSON.parse(data)).status && console.log("任务完成成功")
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function getPoint() {
    return new Promise((resolve => {
        $.get(taskUrl("v1/user/point"), (async (err, resp, data) => {
            try {
                err ? (data = JSON.parse(resp.body), console.log(`${data.error.msg}`)) : safeGet(data) && (data = JSON.parse(data)).status && (data.data.remainPoint >= data.data.oncePoint ? (console.log(`当前赛点：${data.data.remainPoint}/${data.data.oncePoint}，可以兑换京豆，请打开APP兑换`), message += `当前赛点：${data.data.remainPoint}/${data.data.oncePoint}，可以兑换京豆，请打开APP兑换\n`) : (console.log(`当前赛点：${data.data.remainPoint}/${data.data.oncePoint}无法兑换京豆`), message += `当前赛点：${data.data.remainPoint}/${data.data.oncePoint}，无法兑换京豆\n`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function taskUrl(function_id, body = {}) {
    return {
        url: `${JD_API_HOST}${function_id}?timestamp=${(new Date).getTime()+60*(new Date).getTimezoneOffset()*1e3+288e5}`,
        headers: {
            Accept: "*/*",
