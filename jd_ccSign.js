...
async function getCouponConfig() {
    let body = escape(JSON.stringify({
        ...
        sign = await getSign("getCouponConfig", decodeURIComponent(body), uuid),
        url = `${JD_API_HOST}?functionId=getCouponConfig&client=android&clientVersion=10.1.2&uuid=${uuid}&${sign}`;
    return new Promise((async resolve => {
        $.post(taskUrl(url, body), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} getCouponConfig API请求失败，请检查网路重试`);
                else if (data) {
                    let functionId, body;
                    if ((data = JSON.parse(data)).result.couponConfig.signNecklaceDomain)
                        if ("1" === data.result.couponConfig.signNecklaceDomain.roundData.ynSign) console.log("签到失败：今日已签到~");
                        else {
                            let pin = await getsecretPin($.UserName);
                            functionId = "ccSignInNecklace", body = escape(JSON.stringify({
                                childActivityUrl: 'openapp.jdmobile://virtual?params={"category":"jump","des":"couponCenter"}',
                    ...
                    else if ("1" === data.result.couponConfig.signNewDomain.roundData.ynSign) console.log("签到失败：今日已签到~");
                    else {
                        let pin = await getsecretPin($.UserName);
                        functionId = "ccSignInNew", body = escape(JSON.stringify({
                            childActivityUrl: 'openapp.jdmobile://virtual?params={"category":"jump","des":"couponCenter"}',
                            monitorRefer: "appClient",
                            monitorSource: "cc_sign_android_index_config",
                            pageClickKey: "Coupons_GetCenter",
                    ...
                    functionId && body && await ccSign(functionId, body)
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
async function ccSign(functionId, body) {
    let uuid = randomString(16),
        ...
        url = `${JD_API_HOST}?functionId=${functionId}&client=android&clientVersion=10.1.2&uuid=${uuid}&${sign}`;
    return new Promise((async resolve => {
        $.post(taskUrl(url, body), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} ccSign API请求失败，请检查网路重试`)) : data && ("0" === (data = JSON.parse(data)).busiCode ? console.log("ccSignInNew" === functionId ? `签到成功：获得 ${data.result.signResult.signData.amount} 红包` : `签到成功：获得 ${data.result.signResult.signData.necklaceScore} 点点券，${data.result.signResult.signData.amount}`) : console.log(`签到失败：${data.message}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
