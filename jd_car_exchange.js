...
})), process.env.JD_DEBUG && "false" === process.env.JD_DEBUG && (console.log = () => {})) : cookiesArr = [$.getdata("CookieJD"), $.getdata("CookieJD2"), ...jsonParse($.getdata("CookiesJD") || "[]").map((item => item.cookie))].filter((item => !!item)), Date.prototype.Format = function (fmt) {
    var o = {
        "M+": this.getMonth() + 1,
        "d+": this.getDate(),
        "h+": this.getHours(),
...
async function wait(starttime = 59.9) {
    ...
    if (1 == $.index && nowtime < starttime) {
        const sleeptime = 1e3 * (starttime - nowtime);
        console.log("等待时间 " + sleeptime / 1e3), await $.wait(sleeptime)
    }
}
...
function exchange() {
    return new Promise((resolve => {
        $.get(taskUrl("v1/user/exchange/bean/check"), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试\n`)) : safeGet(data) && console.debug("exchange:", data)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function Env(t, e) {
    ...
    if (cookiesArr[0])
        for (let i = 0; i < cookiesArr.length; i++) cookiesArr[i] && (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.isLogin = !0, $.nickName = "", message = "", console.log(`=====京东账号${$.index} ${$.UserName}=====`), await wait(), await exchange(), console.log(`请求兑换API后时间 ${(new Date).Format("yyyy-MM-dd hh:mm:ss | S")}`));
