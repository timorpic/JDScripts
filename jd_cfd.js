! function (t, r) {
    ...
    var r, e, i, n, o, s, a, t = t || function (t, r) {
    ...
    return function () {
        ...
        }(Math), e = (r = t).lib, i = e.WordArray, n = e.Hasher, o = r.algo, s = [], a = o.SHA1 = n.extend({
            _doReset: function () {
...
const $ = new Env("京喜财富岛_Timorpic"),
    JD_API_HOST = "https://m.jingxi.com/",
    notify = $.isNode() ? require("./sendNotify") : "",
    jdCookieNode = $.isNode() ? require("./jdCookie.js") : "";
$.showLog = !!$.getdata("cfd_showLog") && "true" === $.getdata("cfd_showLog"), $.notifyTime = $.getdata("cfd_notifyTime"), $.result = [], $.shareCodes = [];
let UA, nowTimes, cookiesArr = [],
    cookie = "",
    token = "",
    UAInfo = {};
...
async function cfd() {
    try {
        ...
        if (2 === beginInfo.LeadInfo.dwLeadType) {
            if (console.log("还未开通活动，尝试初始化"), await noviceTask(), await $.wait(2e3), beginInfo = await getUserInfo(!1), 2 === beginInfo.LeadInfo.dwLeadType) return void console.log("初始化失败\n");
            console.log("初始化成功\n")
        }
        console.log("寻宝");
        let XBDetail = beginInfo.XbStatus.XBDetail.filter((x => 0 !== x.dwRemainCnt));
        if (0 !== XBDetail.length) {
            console.log("开始寻宝"), $.break = !1;
            for (let key of Object.keys(XBDetail)) {
                let vo = XBDetail[key];
                if (await $.wait(2e3), await TreasureHunt(vo.strIndex), $.break) break
            }
        } else console.log("暂无宝物");
        await $.wait(2e3), await getTakeAggrPage("sign"), await $.wait(2e3), await getTakeAggrPage("wxsign"), await $.wait(2e3), await GetPropCardCenterInfo(), await $.wait(2e3), await getTakeAggrPage("helpdraw"), console.log(""), await $.wait(2e3);
        for (let key of Object.keys($.info.buildInfo.buildList)) {
            let vo = $.info.buildInfo.buildList[key],
                body = `strBuildIndex=${vo.strBuildIndex}`;
            await getBuildInfo(body, vo), await $.wait(2e3)
        }
        if (console.log("接待贵宾"), $.info.StoryInfo.StoryList) {
            await $.wait(2e3);
            for (let key of Object.keys($.info.StoryInfo.StoryList)) {
                let vo = $.info.StoryInfo.StoryList[key];
                vo.Special ? (console.log(`请贵宾下船，需等待${vo.Special.dwWaitTime}秒`), await specialUserOper(vo.strStoryId, "2", vo.ddwTriggerDay, vo), await $.wait(1e3 * vo.Special.dwWaitTime), await specialUserOper(vo.strStoryId, "3", vo.ddwTriggerDay, vo), await $.wait(2e3)) : console.log("当前暂无贵宾\n")
            }
        } else console.log("当前暂无贵宾\n");
        if (console.log("收藏家"), $.info.StoryInfo.StoryList) {
            await $.wait(2e3);
            for (let key of Object.keys($.info.StoryInfo.StoryList)) {
                let vo = $.info.StoryInfo.StoryList[key];
                vo.Collector ? (console.log("喜欢贝壳的收藏家来了，快去卖贝壳吧~"), await collectorOper(vo.strStoryId, "2", vo.ddwTriggerDay), await $.wait(2e3), await querystorageroom("2"), await $.wait(2e3), await collectorOper(vo.strStoryId, "4", vo.ddwTriggerDay)) : console.log("当前暂无收藏家\n")
            }
        } else console.log("当前暂无收藏家\n");
        if (console.log("美人鱼"), $.info.StoryInfo.StoryList) {
            await $.wait(2e3);
            for (let key of Object.keys($.info.StoryInfo.StoryList)) {
                let vo = $.info.StoryInfo.StoryList[key];
                vo.Mermaid ? 1 === vo.Mermaid.dwIsToday ? (console.log("可怜的美人鱼困在沙滩上了，快去解救她吧~"), await mermaidOper(vo.strStoryId, "1", vo.ddwTriggerDay)) : 0 === vo.Mermaid.dwIsToday && await mermaidOper(vo.strStoryId, "4", vo.ddwTriggerDay) : console.log("当前暂无美人鱼\n")
            }
        } else console.log("当前暂无美人鱼\n");
        await $.wait(2e3), await queryRubbishInfo(), console.log("\n做任务"), await $.wait(2e3), await getActTask(), await $.wait(2e3), await getTaskList(0), await $.wait(2e3), await browserTask(0), await $.wait(2e3), await getTaskList(1), await $.wait(2e3), await browserTask(1), await $.wait(2e3), await getPropTask(), await $.wait(2e3);
        const endInfo = await getUserInfo(!1);
...
function GetPropCardCenterInfo() {
    return new Promise((resolve => {
        $.get(taskUrl("user/GetPropCardCenterInfo"), (async (err, resp, data) => {
            try {
                if (err) console.log(JSON.stringify(err)), console.log(`${$.name} GetPropCardCenterInfo API请求失败，请检查网路重试`);
                else if (0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet)
                    if (console.log("使用道具卡"), 0 === data.cardInfo.dwWorkingType) {
                        $.canuse = !1;
                        for (let key of Object.keys(data.cardInfo.coincard)) {
                            let vo = data.cardInfo.coincard[key];
                            if (vo.dwCardNums > 0) {
                                $.canuse = !0, await UsePropCard(vo.strCardTypeIndex);
                                break
                            }
                        }
                        $.canuse || console.log("无可用道具卡\n")
                    } else console.log("有在使用中的道具卡，跳过使用\n")
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function UsePropCard(strCardTypeIndex) {
    return new Promise((resolve => {
        ...
        $.get(taskUrl("user/UsePropCard", `dwCardType=${dwCardType}&strCardTypeIndex=${encodeURIComponent(strCardTypeIndex)}`), ((err, resp, data) => {
            try {
                if (err) console.log(JSON.stringify(err)), console.log(`${$.name} UsePropCard API请求失败，请检查网路重试`);
                else if (0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet) {
                    let cardName = strCardTypeIndex.split("_")[1];
                    console.log(`使用道具卡【${cardName}】成功\n`)
                } else console.log(`使用道具卡失败：${JSON.stringify(data)}\n`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function TreasureHunt(strIndex) {
    return new Promise((resolve => {
        $.get(taskUrl("user/TreasureHunt", `strIndex=${strIndex}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} TreasureHunt API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet ? 0 === data.AwardInfo.dwAwardType ? console.log(`${data.strAwardDesc}，获得 ${data.AwardInfo.ddwValue} 金币`) : 1 === data.AwardInfo.dwAwardType ? (console.log(`${data.strAwardDesc}，获得 ${data.AwardInfo.ddwValue} 财富`), console.log(JSON.stringify(data))) : 4 === data.AwardInfo.dwAwardType ? console.log(`${data.strAwardDesc}，获得 ${data.AwardInfo.strPrizePrice} 红包`) : console.log(JSON.stringify(data)) : (console.log(`寻宝失败：${data.sErrMsg}`), $.break = !0)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function specialUserOper(strStoryId, dwType, ddwTriggerDay, StoryList) {
    return new Promise((resolve => {
        $.get(taskUrl("story/SpecialUserOper", `strStoryId=${strStoryId}&dwType=${dwType}&triggerType=0&ddwTriggerDay=${ddwTriggerDay}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} SpecialUserOper API请求失败，请检查网路重试`)) : (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]), "2" === dwType ? 0 === data.iRet || "success" === data.sErrMsg ? console.log(`贵宾'${StoryList.Special.strName}'下船成功`) : console.log(`贵宾'${StoryList.Special.strName}'下船失败 ${data.sErrMsg}\n`) : "3" === dwType && (0 === data.iRet || "success" === data.sErrMsg ? console.log(`贵宾'${StoryList.Special.strName}'用餐成功：获得${StoryList.Special.ddwCoin}金币\n`) : console.log(`贵宾'${StoryList.Special.strName}'用餐失败：${data.sErrMsg}\n`)))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function collectorOper(strStoryId, dwType, ddwTriggerDay) {
    return new Promise((resolve => {
        $.get(taskUrl("story/CollectorOper", `strStoryId=${strStoryId}&dwType=${dwType}&ddwTriggerDay=${ddwTriggerDay}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} CollectorOper API请求失败，请检查网路重试`)) : data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
async function mermaidOper(strStoryId, dwType, ddwTriggerDay) {
    return new Promise((async resolve => {
        $.get(taskUrl("story/MermaidOper", `strStoryId=${strStoryId}&dwType=${dwType}&ddwTriggerDay=${ddwTriggerDay}`), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} MermaidOper API请求失败，请检查网路重试`);
                else switch (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]), dwType) {
                    case "1":
                        0 === data.iRet || "success" === data.sErrMsg ? (console.log("开始解救美人鱼"), dwType = "3", await mermaidOper(strStoryId, dwType, ddwTriggerDay), await $.wait(2e3)) : console.log(`开始解救美人鱼失败：${data.sErrMsg}\n`);
                        break;
                    case "2":
                    default:
                        break;
                    case "3":
                        if (0 === data.iRet || "success" === data.sErrMsg) {
                            dwType = "2";
                        ...
                        break;
                    case "4":
                        0 === data.iRet || "success" === data.sErrMsg ? console.log(`昨日解救美人鱼领奖成功：获得${data.Data.Prize.strPrizeName}\n`) : console.log(`昨日解救美人鱼领奖失败：${data.sErrMsg}\n`)
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
async function querystorageroom(dwSceneId) {
    return new Promise((async resolve => {
        $.get(taskUrl("story/querystorageroom"), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} querystorageroom API请求失败，请检查网路重试`);
                else {
                    data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]), console.log("\n卖贝壳");
                    let bags = [];
                    for (let key of Object.keys(data.Data.Office)) {
                        let vo = data.Data.Office[key];
                        bags.push(vo.dwType), bags.push(vo.dwCount)
                    }
                    if (0 !== bags.length) {
                        let strTypeCnt = "";
                        for (let j = 0; j < bags.length; j++) strTypeCnt += j % 2 == 0 ? `${bags[j]}:` : `${bags[j]}|`;
                        await $.wait(2e3), await sellgoods(`strTypeCnt=${strTypeCnt}&dwSceneId=${dwSceneId}`)
...
function sellgoods(body) {
    return new Promise((resolve => {
        $.get(taskUrl("story/sellgoods", body), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} sellgoods API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet ? console.log(`贝壳出售成功：获得${data.Data.ddwCoin}金币 ${data.Data.ddwMoney}财富\n`) : console.log(`贝壳出售失败：${data.sErrMsg}\n`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
        ...
        switch (type) {
            case "sign":
                $.get(taskUrl("story/GetTakeAggrPage"), (async (err, resp, data) => {
                    try {
                        if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} GetTakeAggrPage API请求失败，请检查网路重试`);
                        else {
                            data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]), console.log("\n每日签到");
                            for (let key of Object.keys(data.Data.Sign.SignList)) {
                                ...
                                if (vo.dwDayId === data.Data.Sign.dwTodayId) {
                                    if (1 === vo.dwStatus) {
                                        ...
                                        await rewardSign(body), await $.wait(2e3)
                                    }
                                }
                            }
                        }
                ...
                $.get(taskUrl("story/GetTakeAggrPage", "", 6), (async (err, resp, data) => {
                    try {
                        if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} GetTakeAggrPage API请求失败，请检查网路重试`);
                            ...
                            data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]), console.log("小程序每日签到");
                            for (let key of Object.keys(data.Data.Sign.SignList)) {
                                let vo = data.Data.Sign.SignList[key];
                                if (vo.dwDayId === data.Data.Sign.dwTodayId) {
                                    if (1 === vo.dwStatus) {
                                        ...
                                        const body = `ddwCoin=${vo.ddwCoin}&ddwMoney=${vo.ddwMoney}&dwPrizeType=${vo.dwPrizeType}&strPrizePool=${vo.strPrizePool}&dwPrizeLv=${vo.dwBingoLevel}&strPgUUNum=${token.farm_jstoken}&strPgtimestamp=${token.timestamp}&strPhoneID=${token.phoneid}`;
                                        await rewardSign(body, 6), await $.wait(2e3)
                                    }
                                }
                            }
                        }
                ...
                $.get(taskUrl("story/GetTakeAggrPage"), (async (err, resp, data) => {
                    try {
                        if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} GetTakeAggrPage API请求失败，请检查网路重试`);
                            ...
                            data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]), console.log("领助力奖励");
                            let helpNum = [];
                            for (let key of Object.keys(data.Data.Employee.EmployeeList)) {
                                let vo = data.Data.Employee.EmployeeList[key];
                                1 !== vo.dwStatus && helpNum.push(vo.dwId)
                            }
                            if (0 !== helpNum.length)
                                for (let j = 0; j < helpNum.length; j++) await helpdraw(helpNum[j]), await $.wait(2e3);
                            else console.log("暂无可领助力奖励")
                        }
                    } catch (e) {
                        $.logErr(e, resp)
                    } finally {
...
function rewardSign(body, dwEnv = 7) {
    return new Promise((resolve => {
        $.get(taskUrl("story/RewardSign", body, dwEnv), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} RewardSign API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet || "success" === data.sErrMsg ? data.Data.ddwCoin ? console.log(`签到成功：获得${data.Data.ddwCoin}金币\n`) : data.Data.ddwMoney ? console.log(`签到成功：获得${data.Data.ddwMoney}财富\n`) : data.Data.strPrizeName ? console.log(`签到成功：获得${data.Data.strPrizeName}\n`) : console.log("签到成功：很遗憾未中奖~\n") : console.log(`签到失败：${data.sErrMsg}\n`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function helpdraw(dwUserId) {
    return new Promise((resolve => {
        $.get(taskUrl("story/helpdraw", `dwUserId=${dwUserId}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} helpdraw API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet || "success" === data.sErrMsg ? data.Data.StagePrizeInfo ? console.log(`领取助力奖励成功：获得${data.Data.ddwCoin}金币 ${data.Data.StagePrizeInfo.ddwMoney}财富 ${data.Data.StagePrizeInfo.strPrizeName&&!data.Data.StagePrizeInfo.ddwMoney?data.Data.StagePrizeInfo.strPrizeName:"0元"}红包`) : console.log(`领取助力奖励成功：获得${data.Data.ddwCoin}金币`) : console.log(`领取助力奖励失败：${data.sErrMsg}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
async function queryRubbishInfo() {
    return new Promise((async resolve => {
        $.get(taskUrl("story/QueryRubbishInfo"), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} QueryRubbishInfo API请求失败，请检查网路重试`);
                else if (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]), console.log("倒垃圾"), 0 !== data.Data.StoryInfo.StoryList.length)
                    for (let key of Object.keys(data.Data.StoryInfo.StoryList)) {
                        if (data.Data.StoryInfo.StoryList[key].Rubbish) {
                            await $.wait(2e3);
                            ...
                            } else console.log("获取垃圾信息成功：本次不需要垃圾分类"), 0 === rubbishOperRes.iRet || "success" === rubbishOperRes.sErrMsg ? console.log(`倒垃圾成功：获得${rubbishOperRes.Data.ThrowRubbish.ddwCoin}金币\n`) : console.log(`倒垃圾失败：${rubbishOperRes.sErrMsg}\n`)
        ...
        switch (dwType) {
            case "1":
                $.get(taskUrl("story/RubbishOper", "dwType=1&dwRewardType=0"), ((err, resp, data) => {
                    try {
                        err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} RubbishOper API请求失败，请检查网路重试`)) : data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])
                    } catch (e) {
                        $.logErr(e, resp)
                    } finally {
                        resolve(data)
...
async function getActTask(type = !0) {
    return new Promise((async resolve => {
        $.get(taskUrl("story/GetActTask"), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} GetActTask API请求失败，请检查网路重试`);
                else if (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]), type) {
                    for (let key of Object.keys(data.Data.TaskList)) {
                        ...
                        if ([1, 2].includes(vo.dwOrderId) && vo.dwCompleteNum !== vo.dwTargetNum && vo.dwTargetNum < 10) {
                            ...
                            for (let i = vo.dwCompleteNum; i < vo.dwTargetNum; i++) console.log(`【🐮牛牛任务】${vo.strTaskName} 进度：${i+1}/${vo.dwTargetNum}`), await doTask(vo.ddwTaskId, 2), await $.wait(2e3)
                        }
                    }
                    data = await getActTask(!1);
                    for (let key of Object.keys(data.Data.TaskList)) {
                        let vo = data.Data.TaskList[key];
                        vo.dwCompleteNum >= vo.dwTargetNum && 1 !== vo.dwAwardStatus && (await awardActTask("Award", vo), await $.wait(2e3))
                    }(data = await getActTask(!1)).Data.dwCompleteTaskNum >= data.Data.dwTotalTaskNum ? 4 !== data.Data.dwStatus ? (console.log("【🐮牛牛任务】已做完，去开启宝箱"), await awardActTask("story/ActTaskAward"), await $.wait(2e3)) : console.log("【🐮牛牛任务】已做完，宝箱已开启") : console.log("【🐮牛牛任务】未全部完成，无法开启宝箱\n")
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
        ...
        switch (function_path) {
            case "Award":
                $.get(taskListUrl(function_path, `taskId=${ddwTaskId}`, "jxbfddch"), ((err, resp, data) => {
                    try {
                        if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} awardActTask API请求失败，请检查网路重试`);
                        else {
                            const {
                                msg: msg,
                                ret: ret,
                            ...
                            let str = "";
                            str = -1 !== msg.indexOf("活动太火爆了") ? "任务为成就任务或者未到任务时间" : msg + prizeInfo ? `获得金币 ¥ ${JSON.parse(prizeInfo).ddwCoin}` : "", console.log(`【🐮领牛牛任务奖励】${strTaskName} ${str}\n${$.showLog?data:""}`)
                        }
                    } catch (e) {
                        $.logErr(e, resp)
                    } finally {
            ...
            case "story/ActTaskAward":
                $.get(taskUrl(function_path), ((err, resp, data) => {
                    try {
                        err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} awardActTask API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet || "success" === data.sErrMsg ? console.log(`【🐮牛牛任务】开启宝箱成功：获得财富 ¥ ${data.Data.ddwBigReward}\n`) : console.log(`【🐮牛牛任务】开启宝箱失败：${data.sErrMsg}\n`)
                    } catch (e) {
                        $.logErr(e, resp)
                    } finally {
                        resolve()
...
async function getBuildInfo(body, buildList, type = !0) {
    let twobody = body;
    return new Promise((async resolve => {
        $.get(taskUrl("user/GetBuildInfo", body), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} GetBuildInfo API请求失败，请检查网路重试`);
                else if (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]), type) {
                    let buildNmae;
                    ...
                    0 === data.dwBuildLvl && (console.log("创建建筑"), console.log(`【${buildNmae}】当前建筑还未创建，开始创建`), await createbuilding(`strBuildIndex=${data.strBuildIndex}`, buildNmae), await $.wait(2e3), data = await getBuildInfo(twobody, buildList, !1), await $.wait(2e3)), console.log("收金币");
                    const body = `strBuildIndex=${data.strBuildIndex}&dwType=1`;
                    let collectCoinRes = await collectCoin(body);
                    if (console.log(`【${buildNmae}】收集${collectCoinRes.ddwCoin}金币`), await $.wait(3e3), await getUserInfo(!1), console.log("升级建筑"), console.log(`【${buildNmae}】当前等级：${buildList.dwLvl}`), console.log(`【${buildNmae}】升级需要${data.ddwNextLvlCostCoin}金币，保留升级需要的3倍${3*data.ddwNextLvlCostCoin}金币，当前拥有${$.info.ddwCoinBalance}金币`), data.dwCanLvlUp > 0 && $.info.ddwCoinBalance >= 3 * data.ddwNextLvlCostCoin) {
                        ...
                        0 === buildLvlUpRes.iRet ? console.log(`【${buildNmae}】升级成功：获得${data.ddwLvlRich}财富\n`) : console.log(`【${buildNmae}】升级失败：${buildLvlUpRes.sErrMsg}\n`)
...
function collectCoin(body) {
    return new Promise((resolve => {
        $.get(taskUrl("user/CollectCoin", body), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} CollectCoin API请求失败，请检查网路重试`)) : data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function buildLvlUp(body) {
    return new Promise((resolve => {
        $.get(taskUrl("user/BuildLvlUp", body), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} BuildLvlUp API请求失败，请检查网路重试`)) : data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function createbuilding(body, buildNmae) {
    return new Promise((async resolve => {
        $.get(taskUrl("user/createbuilding", body), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} createbuilding API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet && console.log(`【${buildNmae}】创建成功`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
function helpByStage(shareCodes) {
    return new Promise((resolve => {
        $.get(taskUrl("story/helpbystage", `strShareId=${shareCodes}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} helpbystage API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])).iRet || "success" === data.sErrMsg ? console.log(`助力成功：获得${data.Data.GuestPrizeInfo.strPrizeName}`) : 2235 === data.iRet || "今日助力次数达到上限，明天再来帮忙吧~" === data.sErrMsg ? (console.log(`助力失败：${data.sErrMsg}`), $.canHelp = !1) : 2232 === data.iRet || "分享链接已过期" === data.sErrMsg ? (console.log(`助力失败：${data.sErrMsg}`), $.delcode = !0) : 9999 === data.iRet || "您还没有登录，请先登录哦~" === data.sErrMsg ? (console.log(`助力失败：${data.sErrMsg}`), $.canHelp = !1) : 2229 === data.iRet || "助力失败啦~" === data.sErrMsg ? (console.log("助力失败：您的账号已黑"), $.canHelp = !1) : 2190 === data.iRet || "达到助力上限" === data.sErrMsg ? (console.log(`助力失败：${data.sErrMsg}`), $.delcode = !0) : console.log(`助力失败：${data.sErrMsg}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getUserInfo(showInvite = !0) {
    return new Promise((async resolve => {
        $.get(taskUrl("user/QueryUserInfo", `ddwTaskId=&strShareId=&strMarkList=${encodeURIComponent("guider_step,collect_coin_auth,guider_medal,guider_over_flag,build_food_full,build_sea_full,build_shop_full,build_fun_full,medal_guider_show,guide_guider_show,guide_receive_vistor,daily_task,guider_daily_task")}&strPgUUNum=${token.farm_jstoken}&strPgtimestamp=${token.timestamp}&strPhoneID=${token.phoneid}`), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} QueryUserInfo API请求失败，请检查网路重试`);
                else {
                    data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]);
                    const {
                        buildInfo: buildInfo = {},
                    ...
                    showInvite && (console.log(`获取用户信息：${sErrMsg}\n${$.showLog?data:""}`), console.log(`\n当前等级:${dwLandLvl},金币:${ddwCoinBalance},财富值:${ddwRichBalance},连续营业天数:${Business.dwBussDayNum},离线收益:${Business.ddwCoin}\n`)), showInvite && strMyShareId && (console.log("财富岛好友互助码每次运行都变化,旧的当天有效"), console.log(`\n【京东账号${$.index}（${$.UserName}）的${$.name}好友互助码】${strMyShareId}`), $.shareCodes.push(strMyShareId)), $.info = {
                        ...$.info,
                        buildInfo: buildInfo,
                        ddwRichBalance: ddwRichBalance,
                        ddwCoinBalance: ddwCoinBalance,
...
function getPropTask() {
    return new Promise((resolve => {
        $.get(taskUrl("story/GetPropTask"), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} getPropTask API请求失败，请检查网路重试`);
                else {
                    data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1]);
                    for (let key of Object.keys(data.Data.TaskList)) {
                        let vo = data.Data.TaskList[key];
                        vo.dwCompleteNum < vo.dwTargetNum ? (await doTask(vo.ddwTaskId, 3), await $.wait(2e3)) : 1 !== vo.dwAwardStatus && (console.log(`【${vo.strTaskName}】已完成，去领取奖励`), await $.wait(2e3), await awardTask(2, vo))
                    }
                }
            } catch (e) {
                $.logErr(e, resp)
        ...
        switch (taskType) {
            case 0:
                $.get(taskListUrl("GetUserTaskStatusList"), (async (err, resp, data) => {
                    try {
                        if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} GetUserTaskStatusList 日常任务 API请求失败，请检查网路重试`);
                        else {
                            const {
                                ret: ret,
                                data: {
                                    userTaskStatusList: userTaskStatusList = []
                                } = {},
                                msg: msg
                            } = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1]);
                            $.allTask = userTaskStatusList.filter((x => 1 !== x.awardStatus && 1 === x.taskCaller)), 0 === $.allTask.length ? console.log("【📆日常任务】已做完") : console.log(`获取【📆日常任务】列表 ${msg}，总共${$.allTask.length}个任务！\n${$.showLog?data:""}`)
                        }
                    } catch (e) {
                        $.logErr(e, resp)
                    } finally {
            ...
            case 1:
                $.get(taskListUrl("GetUserTaskStatusList"), (async (err, resp, data) => {
                    try {
                        if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} GetUserTaskStatusList 成就任务 API请求失败，请检查网路重试`);
                        else {
                            const {
                                ret: ret,
                                data: {
                            ...
                            $.allTask = userTaskStatusList.filter((x => x.completedTimes >= x.targetTimes && 1 !== x.awardStatus && 2 === x.taskCaller)), 0 === $.allTask.length ? console.log("【🎖成就任务】没有可领奖的任务\n") : console.log(`获取【🎖成就任务】列表 ${msg}，总共${$.allTask.length}个任务！\n${$.showLog?data:""}`)
                        }
                    } catch (e) {
                        $.logErr(e, resp)
                    } finally {
        ...
        switch (taskType) {
            case 0:
                for (let i = 0; i < $.allTask.length; i++) {
                    ...
                    for (let i = start; i < end; i++) console.log(`【📆日常任务】${taskinfo.taskName} 进度：${i+1}/${end}`), await doTask(taskinfo.taskId), await $.wait(2e3);
            ...
            case 1:
                for (let i = 0; i < $.allTask.length; i++) {
                    ...
                    console.log(`开始第${i+1}个【🎖成就任务】${taskinfo.taskName}\n`), taskinfo.completedTimes < taskinfo.targetTimes ? console.log(`【领成就奖励】${taskinfo.taskName} 该成就任务未达到门槛\n`) : (await awardTask(1, taskinfo), await $.wait(2e3))
                }
        }
        resolve()
    }))
...
function doTask(taskId, type = 1) {
    return new Promise((async resolve => {
        let bizCode = "jxbfd";
        2 === type && (bizCode = "jxbfddch"), 3 === type && (bizCode = "jxbfdprop"), $.get(taskListUrl("DoTask", `taskId=${taskId}`, bizCode), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} DoTask API请求失败，请检查网路重试`)) : data = JSON.parse(data.replace(/\n/g, "").match(new RegExp(/jsonpCBK.?\((.*);*\)/))[1])
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
        ...
        switch (taskType) {
            case 0:
                $.get(taskListUrl("Award", `taskId=${taskId}`), ((err, resp, data) => {
                    try {
                        if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} Award API请求失败，请检查网路重试`);
                        else {
                            const {
                                msg: msg,
                                ret: ret,
                            ...
                            let str = "";
                            str = -1 !== msg.indexOf("活动太火爆了") ? "任务为成就任务或者未到任务时间" : msg + prizeInfo ? `获得金币 ¥ ${JSON.parse(prizeInfo).ddwCoin}` : "", console.log(`【领日常奖励】${taskName} ${str}\n${$.showLog?data:""}`)
                        }
                    } catch (e) {
                        $.logErr(e, resp)
                    } finally {
            ...
            case 1:
                $.get(taskListUrl("Award", `taskId=${taskId}`), ((err, resp, data) => {
                    try {
                        if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} AchieveAward API请求失败，请检查网路重试`);
                        else {
                            const {
                                msg: msg,
                                ret: ret,
                            ...
                            } = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1]); - 1 !== msg.indexOf("活动太火爆了") ? console.log("活动太火爆了") : console.log(`【领成就奖励】${taskName} 获得财富值 ¥ ${JSON.parse(prizeInfo).ddwMoney}\n${$.showLog?data:""}`)
                        }
                    } catch (e) {
                        $.logErr(e, resp)
                    } finally {
                ...
                $.get(taskListUrl("Award", `taskId=${ddwTaskId}`, "jxbfdprop"), ((err, resp, data) => {
                    try {
                        if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} Award API请求失败，请检查网路重试`);
                            ...
                            } = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1]); - 1 !== msg.indexOf("活动太火爆了") ? console.log("活动太火爆了") : console.log(`【领卡片奖励】${strTaskName} 获得 ${JSON.parse(prizeInfo).CardInfo.CardList[0].strCardName}\n${$.showLog?data:""}`)
                        }
                    } catch (e) {
                        $.logErr(e, resp)
                    } finally {
...
function taskUrl(function_path, body = "", dwEnv = 7) {
    ...
    return url += `&_stk=${getStk(url)}`, url += `&_ste=1&h5st=${decrypt(Date.now(),"","",url)}&_=${Date.now()+2}&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(Math.floor(26*Math.random())+"A".charCodeAt(0))}&g_ty=ls`, {
        url: url,
        headers: {
            Host: "m.jingxi.com",
            Accept: "*/*",
        ...
        if ($.result.length) {
            if ($.notifyTime) {
                const notifyTimes = $.notifyTime.split(",").map((x => x.split(":"))),
                    now = $.time("HH:mm").split(":");
                console.log(`\n${JSON.stringify(notifyTimes)}`), console.log(`\n${JSON.stringify(now)}`), notifyTimes.some((x => x[0] === now[0] && (!x[1] || x[1] === now[1]))) && $.msg($.name, "", `${$.result.join("\n")}`)
            } else $.msg($.name, "", `${$.result.join("\n")}`);
            $.isNode() && process.env.CFD_NOTIFY_CONTROL && await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName}`, `${$.result.join("\n")}`)
        }
        resolve()
    }))
}
...
function readShareCode() {
    return new Promise((async resolve => {
        $.get({
            url: "https://transfer.nz.lu/cfd",
            timeout: 3e4
        }, ((err, resp, data) => {
            try {
                err ? (console.log(JSON.stringify(err)), console.log(`${$.name} readShareCode API请求失败，请检查网路重试`)) : data && (console.log(`\n随机取${randomCount}个码放到您固定的互助码后面(不影响已有固定互助)`), data = JSON.parse(data))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function uploadShareCode(code) {
    return new Promise((async resolve => {
        $.post({
            url: `https://transfer.nz.lu/upload/cfd?code=${code}&ptpin=${encodeURIComponent(encodeURIComponent($.UserName))}`,
            timeout: 3e4
        }, ((err, resp, data) => {
            try {
                err ? (console.log(JSON.stringify(err)), console.log(`${$.name} uploadShareCode API请求失败，请检查网路重试`)) : data && ("OK" === data ? console.log("已自动提交助力码\n") : "error" === data ? console.log("助力码格式错误，乱玩API是要被打屁屁的~\n") : "full" === data ? console.log("车位已满，请等待下一班次\n") : "exist" === data ? console.log("助力码已经提交过了~\n") : "not in whitelist" === data ? console.log("提交助力码失败，此用户不在白名单中\n") : console.log(`未知错误：${data}\n`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function shareCodesFormat() {
    return new Promise((async resolve => {
        $.newShareCodes = [], $.newShareCodes = [...new Set([...$.shareCodes, ...$.strMyShareIds])], console.log(`您将要助力的好友${JSON.stringify($.newShareCodes)}`), resolve()
    }))
}
            ...
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log("request_algo 签名参数API请求失败，请检查网路重试");
                    ...
                    if (200 === (data = JSON.parse(data)).status) {
                        ...
                        enCryptMethodJDString && ($.enCryptMethodJD = new Function(`return ${enCryptMethodJDString}`)()), console.log("获取签名参数成功！"), console.log(`fp: ${$.fingerprint}`), console.log(`token: ${$.token}`), console.log(`enCryptMethodJD: ${enCryptMethodJDString}`)
                    } else console.log(`fp: ${$.fingerprint}`), console.log("request_algo 签名参数API请求失败:");
                else console.log("京东服务器返回空数据")
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
function decrypt(time, stk, type, url) {
    if (stk = stk || (url ? getUrlData(url, "_stk") : "")) {
        const timestamp = new Date(time).Format("yyyyMMddhhmmssSSS");
        let hash1 = "";
        if ($.fingerprint && $.token && $.enCryptMethodJD) hash1 = $.enCryptMethodJD($.token, $.fingerprint.toString(), timestamp.toString(), $.appId.toString(), $.CryptoJS).toString($.CryptoJS.enc.Hex);
        else {
            const random = "5gkjB6SpmC9s";
            $.token = "tk01wcdf61cb3a8nYUtHcmhSUFFCfddDPRvKvYaMjHkxo6Aj7dhzO+GXGFa9nPXfcgT+mULoF1b1YIS1ghvSlbwhE0Xc", $.fingerprint = 5287160221454703;
...
function getUrlData(url, name) {
    if ("undefined" != typeof URL) {
        let data = new URL(url).searchParams.get(name);
        return data || ""
    } {
        const vars = url.match(/\?.*/)[0].substring(1).split("&");
        for (let i = 0; i < vars.length; i++) {
            if (vars[i].split("=")[0] === name) return vars[i].substr(vars[i].indexOf("=") + 1)
        }
        return ""
    }
}
...
$.appId = 10032, $.isNode() ? (Object.keys(jdCookieNode).forEach((item => {
    cookiesArr.push(jdCookieNode[item])
})), process.env.JD_DEBUG && "false" === process.env.JD_DEBUG && (console.log = () => {}), JSON.stringify(process.env).indexOf("GITHUB") > -1 && process.exit(0)) : cookiesArr = [$.getdata("CookieJD"), $.getdata("CookieJD2"), ...jsonParse($.getdata("CookiesJD") || "[]").map((item => item.cookie))].filter((item => !!item)), (async () => {
    if (!cookiesArr[0]) return void $.msg($.name, "【提示】请先获取京东账号一cookie\n直接使用NobyDa的京东签到获取", "https://bean.m.jd.com/bean/signIndex.action", {
        "open-url": "https://bean.m.jd.com/bean/signIndex.action"
    });
    $.CryptoJS = $.isNode() ? require("crypto-js") : CryptoJS, await requestAlgo(), await $.wait(1e3);
    for (let i = 0; i < cookiesArr.length; i++)
        if (cookiesArr[i]) {
            if (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.nickName = "", $.isLogin = !0, UA = `jdpingou;iPhone;4.13.0;14.4.2;${randomString(40)};network/wifi;model/iPhone10,2;appBuild/100609;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${98*Math.random+1};pap/JA2019_3111789;brand/apple;supportJDSHWK/1;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148`, UAInfo[$.UserName] = UA, await TotalBean(), console.log(`\n******开始【京东账号${$.index}】${$.nickName||$.UserName}*********\n`), !$.isLogin) {
                $.msg($.name, "【提示】cookie已失效", `京东账号${$.index} ${$.nickName||$.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {
                    "open-url": "https://bean.m.jd.com/bean/signIndex.action"
                }), $.isNode() && await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
                continue
            }
            $.allTask = [], $.info = {}, token = await getJxToken(), await cfd(), await $.wait(2e3)
        } let res = await getAuthorShareCode("https://sub.timor.icu/JDscripts/cfd.json");
    res || ($.http.get({
        url: "https://sub.timor.icu/JDscripts/cfd.json"
    }).then((resp => {})).catch((e => console.log("刷新CDN异常", e))), await $.wait(1e3), res = await getAuthorShareCode("https://sub.timor.icu/JDscripts/cfd.json")), $.strMyShareIds = [...res && res.shareId || []], await shareCodesFormat();
    for (let i = 0; i < cookiesArr.length && (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.canHelp = !0, UA = UAInfo[$.UserName], $.newShareCodes && $.newShareCodes.length); i++) {
        console.log("\n开始互助\n");
        for (let j = 0; j < $.newShareCodes.length && $.canHelp; j++) console.log(`账号${$.UserName} 去助力 ${$.newShareCodes[j]}`), $.delcode = !1, await helpByStage($.newShareCodes[j]), await $.wait(2e3), $.delcode && ($.newShareCodes.splice(j, 1), j--)
    }
    await showMsg()
})().catch((e => $.logErr(e))).finally((() => $.done())), Date.prototype.Format = function (fmt) {
    var n = this,
        d = fmt,
        l = {
            "M+": n.getMonth() + 1,
...
! function (_0x30e78a, _0x12a1c3, _0x4ca71c) {
    (function (_0x59c439, _0x435a06, _0x70e6be, _0x39d363, _0x31edda) {
        if ((_0x435a06 >>= 8) < _0x59c439) {
            for (; --_0x59c439;) _0x39d363 = _0x30e78a.shift(), _0x435a06 === _0x59c439 ? (_0x435a06 = _0x39d363, _0x70e6be = _0x30e78a.pop()) : _0x435a06 && _0x70e6be.replace(/[WIeSnSTdXuZbEnD=]/g, "") === _0x435a06 && _0x30e78a.push(_0x39d363);
            _0x30e78a.push(_0x30e78a.shift())
        }
    })(++_0x12a1c3, 28160)
}(_0x2cf9, 110);
var _0x5108 = function (_0x4dc255, _0x3cb8bc) {
    ...
    if (void 0 === _0x5108.xFLNEr) {
        ! function () {
            var _0xfc2aa4 = "undefined" != typeof window ? window : "object" == typeof process && "function" == typeof require && "object" == typeof global ? global : this;
            _0xfc2aa4.atob || (_0xfc2aa4.atob = function (_0x509ed4) {
                for (var _0x5a7e73, _0x42fadc, _0x2e5ed8 = String(_0x509ed4).replace(/=+$/, ""), _0x5f2c3c = 0, _0x50b6c7 = 0, _0x2de292 = ""; _0x42fadc = _0x2e5ed8.charAt(_0x50b6c7++); ~_0x42fadc && (_0x5a7e73 = _0x5f2c3c % 4 ? 64 * _0x5a7e73 + _0x42fadc : _0x42fadc, _0x5f2c3c++ % 4) ? _0x2de292 += String.fromCharCode(255 & _0x5a7e73 >> (-2 * _0x5f2c3c & 6)) : 0) _0x42fadc = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".indexOf(_0x42fadc);
                return _0x2de292
            })
        }();
        _0x5108.NgRmMn = function (_0x517424, _0x3cb8bc) {
            for (var _0x50c70e, _0x5bb1d7 = [], _0x204abf = 0, _0x376d53 = "", _0x19ba11 = "", _0x2212a4 = 0, _0x34e1ad = (_0x517424 = atob(_0x517424)).length; _0x2212a4 < _0x34e1ad; _0x2212a4++) _0x19ba11 += "%" + ("00" + _0x517424.charCodeAt(_0x2212a4).toString(16)).slice(-2);
            _0x517424 = decodeURIComponent(_0x19ba11);
            for (var _0x5372ab = 0; _0x5372ab < 256; _0x5372ab++) _0x5bb1d7[_0x5372ab] = _0x5372ab;
            for (_0x5372ab = 0; _0x5372ab < 256; _0x5372ab++) _0x204abf = (_0x204abf + _0x5bb1d7[_0x5372ab] + _0x3cb8bc.charCodeAt(_0x5372ab % _0x3cb8bc.length)) % 256, _0x50c70e = _0x5bb1d7[_0x5372ab], _0x5bb1d7[_0x5372ab] = _0x5bb1d7[_0x204abf], _0x5bb1d7[_0x204abf] = _0x50c70e;
            _0x5372ab = 0, _0x204abf = 0;
            for (var _0x30875f = 0; _0x30875f < _0x517424.length; _0x30875f++) _0x204abf = (_0x204abf + _0x5bb1d7[_0x5372ab = (_0x5372ab + 1) % 256]) % 256, _0x50c70e = _0x5bb1d7[_0x5372ab], _0x5bb1d7[_0x5372ab] = _0x5bb1d7[_0x204abf], _0x5bb1d7[_0x204abf] = _0x50c70e, _0x376d53 += String.fromCharCode(_0x517424.charCodeAt(_0x30875f) ^ _0x5bb1d7[(_0x5bb1d7[_0x5372ab] + _0x5bb1d7[_0x204abf]) % 256]);
            return _0x376d53
        }, _0x5108.CiKmfm = {}, _0x5108.xFLNEr = !0
    }
    var _0x15f777 = _0x5108.CiKmfm[_0x4dc255];
    return void 0 === _0x15f777 ? (void 0 === _0x5108.GhDaFS && (_0x5108.GhDaFS = !0), _0x2e664b = _0x5108.NgRmMn(_0x2e664b, _0x3cb8bc), _0x5108.CiKmfm[_0x4dc255] = _0x2e664b) : _0x2e664b = _0x15f777, _0x2e664b
};
...
function getJxToken() {
    ...
    function _0x23cb77(_0x378208) {
        let _0x36ad34 = _0x3565bd[_0x5108("1", "cqej")],
            _0x3ba0b7 = "";
        for (let _0x24b162 = 0; _0x3565bd[_0x5108("2", "1#C#")](_0x24b162, _0x378208); _0x24b162++) _0x3ba0b7 += _0x36ad34[_0x3565bd[_0x5108("3", "Hq%O")](parseInt, _0x3565bd[_0x5108("4", "U*Pv")](Math.random(), _0x36ad34[_0x5108("5", "8QnT")]))];
        return _0x3ba0b7
    }
    return new Promise((_0x2ef875 => {
        let _0x9ac908 = _0x3565bd[_0x5108("6", "x)1A")](_0x23cb77, 40),
            _0x256650 = (+new Date)[_0x5108("7", "U*Pv")]();
        cookie[_0x5108("8", "8QnT")](/pt_pin=([^; ]+)(?=;?)/) || (console.log(_0x5108("9", "Hq%O")), _0x3565bd.GoCYd(_0x2ef875, null));
        let _0x4e1006 = cookie[_0x5108("a", "8#od")](/pt_pin=([^; ]+)(?=;?)/)[1],
            _0x57bff6 = $.md5("" + decodeURIComponent(_0x4e1006) + _0x256650 + _0x9ac908 + "tPOamqCuk9NLgVPAljUyIHcPRmKlVxDy")[_0x5108("b", "]OsH")]();
