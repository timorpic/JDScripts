const $ = new Env("宠汪汪🐕喂食_Timorpic"),
    zooFaker = require("./utils/JDJRValidator_Pure");
...
let FEED_NUM = 1 * $.getdata("joyFeedCount") || 10;
...
function ThreeMeals() {
    return new Promise((resolve => {
        ...
        $.get(options, (async (err, resp, data) => {
            try {
                (data = JSON.parse(data)).success && "received" === data.errorCode && (console.log("三餐结果领取成功"), message += `【三餐】领取成功，获得${data.data}g狗粮\n`)
            } catch (e) {
                $.logErr(resp, e)
            } finally {
                resolve(data)
...
function Env(t, e) {
    ...
    if (cookiesArr[0]) {
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                if (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.isLogin = !0, $.nickName = "", await $.wait(100), await TotalBean(), console.log(`\n*****开始【京东账号${$.index}】${$.nickName||$.UserName}******\n`), !$.isLogin) {
                    $.msg($.name, "【提示】cookie已失效", `京东账号${$.index} ${$.nickName||$.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {
                        "open-url": "https://bean.m.jd.com/bean/signIndex.action"
                    }), $.isNode() && await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
                    continue
                }
                message = "", subTitle = "", $.isNode() && process.env.JOY_FEED_COUNT && ([0, 10, 20, 40, 80].indexOf(1 * process.env.JOY_FEED_COUNT) > -1 ? FEED_NUM = process.env.JOY_FEED_COUNT ? 1 * process.env.JOY_FEED_COUNT : FEED_NUM : console.log("您输入的 JOY_FEED_COUNT 为非法数字，请重新输入")), $.validate = "", await feedPets(FEED_NUM), await ThreeMeals(), await showMsg()
            }
