...
const $ = new Env("京喜签到_Timorpic"),
    JD_API_HOST = "https://m.jingxi.com/",
    notify = $.isNode() ? require("./sendNotify") : "",
    jdCookieNode = $.isNode() ? require("./jdCookie.js") : "";
let message, UA, jdNotify = !0,
    cookiesArr = [],
    cookie = "",
    UAInfo = {};
...
function signhb(type = 1) {
    return new Promise((resolve => {
        $.get(taskUrl("fanxiantask/signhb/query"), (async (err, resp, data) => {
            try {
                if (err) console.log(JSON.stringify(err)), console.log(`${$.name} query签到 API请求失败，请检查网路重试`);
                else {
                    ...
                    $.domax = domax;
                    let helpNum = 0;
                    switch (helppic && (helpNum = helppic.split(";").length - 1), type) {
                        case 1:
                            if (1 === status) {
                                let max = !1;
                                helpNum == domax && (max = !0), $.shareCodes.push({
                                    use: $.UserName,
                                    smp: smp,
                                    num: helpNum || 0,
                                    max: max
                        ...
                        case 2:
                            for (let key of Object.keys(signlist)) {
                                let vo = signlist[key];
                                1 === vo.istoday && (1 === vo.status && 1 === data.todaysign ? (console.log("今日已签到"), $.canHelp = !1) : console.log("今日未签到"))
                            }
                            console.log(`【签到互助码】${smp}`), helpNum && console.log(`已有${helpNum}人助力`);
                            for (let i = 0; i < commontask.length; i++) commontask[i].task && 2 != commontask[i].status && $.commonlist.push(commontask[i].task);
                            console.log(`可开启宝箱${data.baoxiang_left}个`), $.bxNum.push({
...
function helpSignhb(smp = "") {
    return new Promise((resolve => {
        $.get(taskUrl("fanxiantask/signhb/query", `signhb_source=1000&smp=${smp}&type=1`, "signhb_source,smp,type"), (async (err, resp, data) => {
            try {
                if (err) console.log(JSON.stringify(err)), console.log(`${$.name} query助力 API请求失败，请检查网路重试`);
                else {
                    data = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1]);
                    const {
                        signlist: signlist = []
                    ...
                    for (let key of Object.keys(signlist)) {
                        let vo = signlist[key];
                        1 === vo.istoday && (1 === vo.status && 1 === data.todaysign || (console.log("此账号已黑"), $.black = !0))
                    }
                }
            } catch (e) {
                $.logErr(e, resp)
...
function dotask(task) {
    return new Promise((resolve => {
        $.get(taskUrl("fanxiantask/signhb/dotask", `signhb_source=1000&task=${task}`, "signhb_source,task"), (async (err, resp, data) => {
            try {
                err ? (console.log(JSON.stringify(err)), console.log(`${$.name} dotask API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1])).ret ? console.log(`完成任务 获得${data.sendhb}红包`) : console.log(data.errmsg)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function bxdraw() {
    return new Promise((resolve => {
        $.get(taskUrl("fanxiantask/signhb/bxdraw"), (async (err, resp, data) => {
            try {
                err ? (console.log(JSON.stringify(err)), console.log(`${$.name} bxdraw API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1])).ret ? console.log(`开启宝箱 获得${data.sendhb}红包`) : console.log(data.errmsg)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function doubleSign() {
    return new Promise((resolve => {
        $.get(taskUrl("double_sign/IssueReward"), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} IssueReward API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data)).retCode ? console.log("双签成功") : console.log(`任务完成失败，错误信息${data.errMsg}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function taskUrl(functionId, body = "", stk) {
    let url = "";
    return body ? (url = `${JD_API_HOST}${functionId}?${body?`${body}&`:""}sceneval=2&g_login_type=1&_=${Date.now()}&_ste=1&callback=jsonpCBKC&g_ty=ls`, url += `&h5st=${decrypt(Date.now(),stk,"",url)}`, stk && (url += `&_stk=${encodeURIComponent(stk)}`)) : url = "double_sign/IssueReward" === functionId ? `${JD_API_HOST}${functionId}?sceneval=2&g_login_type=1&_ste=1&g_ty=ajax` : `${JD_API_HOST}${functionId}?_=${Date.now()}&sceneval=2&g_login_type=1&callback=jsonpCBKC&g_ty=ls`, {
        url: url,
        headers: {
            Cookie: cookie,
            Host: "m.jingxi.com",
...
$.shareCodes = [], $.blackInfo = {}, $.appId = 10028, $.isNode() ? (Object.keys(jdCookieNode).forEach((item => {
    ...
    if ($.CryptoJS = $.isNode() ? require("crypto-js") : CryptoJS, await requestAlgo(), await $.wait(1e3), !cookiesArr[0]) return void $.msg($.name, "【提示】请先获取京东账号一cookie\n直接使用NobyDa的京东签到获取", "https://bean.m.jd.com/bean/signIndex.action", {
        "open-url": "https://bean.m.jd.com/bean/signIndex.action"
    });
    for (let i = 0; i < cookiesArr.length; i++)
        if (cookiesArr[i]) {
            if (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.isLogin = !0, await TotalBean(), !$.isLogin) continue;
            0 === i && console.log("\n正在收集助力码请等待\n"), UA = `jdpingou;iPhone;4.13.0;14.4.2;${randomString(40)};network/wifi;model/iPhone10,2;appBuild/100609;ADID/00000000-0000-0000-0000-000000000000;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${98*Math.random+1};pap/JA2019_3111789;brand/apple;supportJDSHWK/1;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148`, await signhb(1), await $.wait(500), UAInfo[$.UserName] = UA
        } console.log("作者助力码追加至最后!如有剩余次数则给作者助力!感谢!");
    const author = ["91c238932ae6648c5a1d02111aac675c"];
    for (let i = 0; i < author.length; i++) $.shareCodes.push({
        use: "timorpic",
        smp: author[i],
        num: 0,
        max: !1
    ...
    for (let i = 0; i < cookiesArr.length; i++)
        if (cookiesArr[i]) {
            if (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.isLogin = !0, $.nickName = "", message = "", $.commonlist = [], $.bxNum = [], $.black = !1, $.canHelp = !0, await TotalBean(), console.log(`\n******开始【京东账号${$.index}】${$.nickName||$.UserName}*********\n`), !$.isLogin) {
                $.msg($.name, "【提示】cookie已失效", `京东账号${$.index} ${$.nickName||$.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {
                    "open-url": "https://bean.m.jd.com/bean/signIndex.action"
                }), $.isNode() && await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
                continue
            }
            if (UA = UAInfo[$.UserName], await signhb(2), await $.wait(2e3), $.canHelp) {
                if ($.shareCodes && $.shareCodes.length) {
                    console.log("\n开始内部互助\n");
                    for (let j = 0; j < $.shareCodes.length; j++)
                        if ($.shareCodes[j].num != $.domax)
                            if ($.shareCodes[j].use !== $.UserName) {
                                if (console.log(`账号 ${$.UserName} 去助力 ${$.shareCodes[j].use} 的互助码 ${$.shareCodes[j].smp}`), !$.shareCodes[j].max) {
                                    await helpSignhb($.shareCodes[j].smp), await $.wait(2e3), $.black || $.shareCodes[j].num++;
                                    break
                                }
                                console.log("您的好友助力已满")
                            } else console.log("不能助力自己");
                    else $.shareCodes.splice(j, 1), j--
                }
            } else console.log("今日已签到，无法助力好友啦~");
            if ($.black) console.log("此账号已黑");
            else {
                if (await helpSignhb(), $.commonlist && $.commonlist.length) {
                    console.log("开始做红包任务");
                    for (let j = 0; j < $.commonlist.length; j++) await dotask($.commonlist[j]), await $.wait(2e3)
                } else console.log("红包任务已完成");
                if ($.bxNum && $.bxNum.length)
                    for (let j = 0; j < $.bxNum[0].bxNum; j++) await bxdraw(), await $.wait(2e3);
                await doubleSign()
            }
        }
})().catch((e => {
