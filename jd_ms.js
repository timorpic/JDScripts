...
async function jdMs() {
    $.score = 0, await getActInfo(), await getUserInfo(), $.cur = $.score, $.encryptProjectId && await getTaskList(), await getUserInfo(!1)
}
...
function getActInfo() {
    return new Promise((resolve => {
        $.post(taskPostUrl("assignmentList", {}, "appid=jwsp"), ((err, resp, data) => {
            try {
                err ? (console.log(`${err},${jsonParse(resp.body).message}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && 200 === (data = JSON.parse(data)).code && ($.encryptProjectId = data.result.assignmentResult.encryptProjectId, console.log(`活动名称：${data.result.assignmentResult.projectName}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getUserInfo(info = !0) {
    return new Promise((resolve => {
        $.post(taskPostUrl("homePageV2", {}, "appid=SecKill2020"), ((err, resp, data) => {
            try {
                err ? (console.log(`${err},${jsonParse(resp.body).message}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && 2041 === (data = JSON.parse(data)).code && ($.score = data.result.assignment.assignmentPoints || 0, info && console.log(`当前秒秒币${$.score}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
        ...
        $.post(taskPostUrl("queryInteractiveInfo", body), (async (err, resp, data) => {
            try {
                if (err) console.log(`${err},${jsonParse(resp.body).message}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                    ...
                    if (data = JSON.parse(data), $.risk = !1, "0" === data.code)
                        for (let vo of data.assignmentList) {
                            if ($.risk) break;
                            if (vo.completionCnt < vo.assignmentTimesLimit)
                                if (1 === vo.assignmentType) {
                                    if (0 === vo.ext[vo.ext.extraType].length) continue;
                                    for (let i = vo.completionCnt; i < vo.assignmentTimesLimit; ++i) {
                                        ...
                                        await doTask(body), await $.wait(1e3 * vo.ext.waitDuration + 500), body.actionType = 0, await doTask(body)
                                    }
                                } else if (0 === vo.assignmentType)
                                for (let i = vo.completionCnt; i < vo.assignmentTimesLimit; ++i) {
                                    console.log(`去做${vo.assignmentName}任务：${i+1}/${vo.assignmentTimesLimit}`);
                                    let body = {
                                        encryptAssignmentId: vo.encryptAssignmentId,
                                    ...
                                    await doTask(body), await $.wait(1e3)
                                } else if (3 === vo.assignmentType)
                                    for (let i = vo.completionCnt; i < vo.assignmentTimesLimit; ++i) {
                                        console.log(`去做${vo.assignmentName}任务：${i+1}/${vo.assignmentTimesLimit}`);
                                        let body = {
                                            encryptAssignmentId: vo.encryptAssignmentId,
...
function doTask(body) {
    return body = {
        ...
        $.post(taskPostUrl("doInteractiveAssignment", body), ((err, resp, data) => {
            try {
                err ? (console.log(`${err},${jsonParse(resp.body).message}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (data = JSON.parse(data), console.log(data.msg), "风险等级未通过" === data.msg && ($.risk = 1))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
