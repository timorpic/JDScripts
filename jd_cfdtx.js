...
const $ = new Env("京喜财富岛提现_Timorpic"),
...
let token, nowTimes, UA, cookiesArr = [],
    cookie = "",
    allMessage = "",
    message = "";
async function cfd() {
    try {
        if (nowTimes = new Date((new Date).getTime() + 60 * (new Date).getTimezoneOffset() * 1e3 + 288e5), (11 === nowTimes.getHours() || 23 === nowTimes.getHours()) && 59 === nowTimes.getMinutes()) {
            let nowtime = (new Date).Format("s.S"),
                starttime = $.isNode() ? process.env.CFD_STARTTIME ? 1 * process.env.CFD_STARTTIME : 59.5 : $.getdata("CFD_STARTTIME") ? 1 * $.getdata("CFD_STARTTIME") : 59.5;
            if (nowtime < 59) {
                let sleeptime = 1e3 * (starttime - nowtime);
                console.log(`等待时间 ${sleeptime/1e3}\n`), await sleep(sleeptime)
            }
        }
        $.num % 2 != 0 && (console.log("等待"), await $.wait(2e3));
        if (0 === (await getUserInfo()).Fund.ddwFundTargTm) return void console.log("还未开通活动，请先开通\n");
        console.log("获取提现资格"), await cashOutQuali(), await showMsg()
    } catch (e) {
        $.logErr(e)
    }
}
async function cashOutQuali() {
    return new Promise((async resolve => {
        $.get(taskUrl("user/CashOutQuali", `strPgUUNum=${token.farm_jstoken}&strPgtimestamp=${token.timestamp}&strPhoneID=${token.phoneid}`), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} CashOutQuali API请求失败，请检查网路重试`)) : 0 === (data = JSON.parse(data)).iRet || 2034 === data.iRet ? (console.log(0 === data.iRet ? "获取提现资格成功\n" : `获取提现资格失败：${data.sErrMsg}\n`), console.log("提现\n提现金额：按库存轮询提现，0点场提1元以上，12点场提0.5元以上，12点后不做限制\n"), await userCashOutState()) : console.log(`获取提现资格失败：${data.sErrMsg}\n`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
async function userCashOutState(type = !0) {
    return new Promise((async resolve => {
        $.get(taskUrl("user/UserCashOutState"), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} UserCashOutState API请求失败，请检查网路重试`);
                else if (data = JSON.parse(data), type)
                    if (1 !== data.dwTodayIsCashOut)
                        if (data.ddwUsrTodayGetRich >= data.ddwTodayTargetUnLockRich) {
                            nowTimes = new Date((new Date).getTime() + 60 * (new Date).getTimezoneOffset() * 1e3 + 288e5), nowTimes.getHours() >= 0 && nowTimes.getHours() < 12 ? data.UsrCurrCashList = data.UsrCurrCashList.filter((x => x.ddwMoney / 100 >= 1)) : 12 === nowTimes.getHours() && nowTimes.getMinutes() <= 5 && (data.UsrCurrCashList = data.UsrCurrCashList.filter((x => x.ddwMoney / 100 >= .5)));
                            for (let key of Object.keys(data.UsrCurrCashList).reverse()) {
                                ...
                                if (vo.dwRemain > 0) {
                                    ...
                                    if (0 === cashOutRes.iRet) {
                                        $.money = vo.ddwMoney / 100, console.log(`提现成功：获得${$.money}元`);
                                        break
                                    }
                                    console.log(`提现失败：${cashOutRes.sErrMsg}`), await userCashOutState()
                                } else console.log(`提现失败：${vo.ddwMoney/100}元库存不足`)
                            }
                        } else {
                            console.log("不满足提现条件开始升级建筑");
                            for (let key of Object.keys($.info.buildInfo.buildList)) {
                                let buildNmae, vo = $.info.buildInfo.buildList[key],
                                    body = `strBuildIndex=${vo.strBuildIndex}`,
                                    getBuildInfoRes = await getBuildInfo(body);
                                ...
                                if (console.log("升级建筑"), console.log(`【${buildNmae}】当前等级：${vo.dwLvl}`), console.log(`【${buildNmae}】升级需要${getBuildInfoRes.ddwNextLvlCostCoin}金币，当前拥有${$.info.ddwCoinBalance}金币`), getBuildInfoRes.dwCanLvlUp > 0 && $.info.ddwCoinBalance >= getBuildInfoRes.ddwNextLvlCostCoin) {
                            ...
                            0 === buildLvlUpRes.iRet ? await userCashOutState() : console.log(`今日还未赚够${data.ddwTodayTargetUnLockRich}财富，无法提现`)
                        }
                else console.log("提现失败：今天已经提现过了~")
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function cashOut(ddwMoney, ddwPaperMoney) {
    return new Promise((resolve => {
        $.get(taskUrl("user/CashOut", `ddwMoney=${ddwMoney}&ddwPaperMoney=${ddwPaperMoney}&strPgUUNum=${token.farm_jstoken}&strPgtimestamp=${token.timestamp}&strPhoneID=${token.phoneid}`), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} CashOut API请求失败，请检查网路重试`)) : data = JSON.parse(data)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function getBuildInfo(body) {
    return new Promise((resolve => {
        $.get(taskUrl("user/GetBuildInfo", body), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} GetBuildInfo API请求失败，请检查网路重试`)) : data = JSON.parse(data)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function buildLvlUp(body) {
    return new Promise((resolve => {
        $.get(taskUrl("user/BuildLvlUp", body), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} BuildLvlUp API请求失败，请检查网路重试`)) : data = JSON.parse(data)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function taskUrl(function_path, body = "") {
    ...
    return url += `&h5st=${decrypt(Date.now(),"","",url)}&_=${Date.now()+2}&sceneval=2&g_login_type=1&g_ty=ls`, {
        url: url,
        headers: {
            Cookie: cookie,
            Accept: "*/*",
...
function showMsg() {
    return new Promise((resolve => {
        $.money > 0 ? message += `提现成功：获得${$.money}元` : message += "提现失败：获得空气", $.money > 0 && (allMessage += `【京东账号${$.index}】${$.nickName||$.UserName}\n${message}${$.index,cookiesArr.length,"\n\n"}`), $.msg($.name, "", `【京东账号${$.index}】${$.nickName}\n${message}`), resolve()
    }))
}
...
$.isNode() ? (Object.keys(jdCookieNode).forEach((item => {
...
})), process.env.JD_DEBUG && "false" === process.env.JD_DEBUG && (console.log = () => {}), JSON.stringify(process.env).indexOf("GITHUB") > -1 && process.exit(0)) : cookiesArr = [$.getdata("CookieJD"), $.getdata("CookieJD2"), ...jsonParse($.getdata("CookiesJD") || "[]").map((item => item.cookie))].filter((item => !!item)), $.appId = 10028, Date.prototype.Format = function (fmt) {
    var o = {
        "M+": this.getMonth() + 1,
        "d+": this.getDate(),
        "h+": this.getHours(),
    ...
    if (cookiesArr[0]) {
        ...
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                if (cookie = cookiesArr[i], $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), $.index = i + 1, $.nickName = "", $.isLogin = !0, message = "", await TotalBean(), console.log(`\n******开始【京东账号${$.index}】${$.nickName||$.UserName}*********\n`), !$.isLogin) {
                    $.msg($.name, "【提示】cookie已失效", `京东账号${$.index} ${$.nickName||$.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {
                        "open-url": "https://bean.m.jd.com/bean/signIndex.action"
                    }), $.isNode() && await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
                    continue
                }
                $.num = i, $.info = {}, $.money = 0, UA = `jdpingou;iPhone;4.13.0;14.4.2;${randomString()};network/wifi;model/iPhone10,2;appBuild/100609;ADID/00000000-0000-0000-0000-000000000000;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${98*Math.random+1};pap/JA2019_3111789;brand/apple;supportJDSHWK/1;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148`, token = await getJxToken(), await cfd()
