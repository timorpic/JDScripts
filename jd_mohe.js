...
function addShare(shareId) {
    return new Promise((resolve => {
        ...
        $.post(taskurl(body), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : 200 === (data = JSON.parse(data)).code ? console.log(`助力好友【${data.data}】成功\n`) : console.log(`助力失败：${data.msg}`)
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function homeGoBrowse(type, id) {
    return new Promise((resolve => {
        const body = {
            type: type,
            id: id,
            apiMapping: "/active/homeGoBrowse"
        };
        $.post(taskurl(body), ((err, resp, data) => {
            try {
                data = JSON.parse(data)
...
function taskHomeCoin(type, id) {
    return new Promise((resolve => {
        const body = {
            type: type,
            id: id,
            apiMapping: "/active/taskHomeCoin"
        };
        $.post(taskurl(body), ((err, resp, data) => {
            try {
                data = JSON.parse(data)
...
function getCoin() {
    return new Promise((resolve => {
        $.post(taskurl({
            apiMapping: "/active/getCoin"
        }), ((err, resp, data) => {
            try {
                1001 === (data = JSON.parse(data)).code ? (console.log(data.msg), $.msg($.name, "领取失败", `${data.msg}`), $.done()) : (console.log(`成功领取${data.data}热力值`), resolve(data))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
        ...
        $.post(taskurl({
            ...
            try {
                if (200 === (data = JSON.parse(data)).code) {
                    ...
                    task4.finishNum < task4.totalNum && (await browseProduct(task4.skuId), await $.wait(2e3), await taskCoin(task4.type), await $.wait(2e3)), task1.finishNum < task1.totalNum && (await strollActive(task1.finishNum + 1), await $.wait(2e3), await taskCoin(task1.type), await $.wait(2e3)), task2.finishNum < task2.totalNum && (await followShop(task2.shopId), await $.wait(2e3), await taskCoin(task2.type), await $.wait(2e3)), 2 === task4.state && 2 === task1.state && 2 === task2.state ? (console.log("\n\n----taskList的任务全部做完了---\n\n"), console.log(`分享好友助力 ${task5.finishNum}/${task5.totalNum}\n\n`)) : (console.log("请继续等待,正在做任务,不要退出哦"), await taskList())
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
function browseProduct(skuId) {
    return new Promise((resolve => {
        const body = {
            skuId: skuId,
            apiMapping: "/active/browseProduct"
        };
        $.post(taskurl(body), ((err, resp, data) => {
            try {
                data = JSON.parse(data)
...
function strollActive(index) {
    return new Promise((resolve => {
        const body = {
            activeId: index,
            apiMapping: "/active/strollActive"
        };
        $.post(taskurl(body), ((err, resp, data) => {
            try {
                data = JSON.parse(data)
    ...
    if (200 === coinRes.code) {
        ...
        if (total > need) {
            ...
            for (let i = 0; i < times; i++) {
                ...
                if (200 === lotteryRes.code) console.log(`====抽奖结果====,${JSON.stringify(lotteryRes.data)}`), console.log(lotteryRes.data.name), console.log(lotteryRes.data.beanNum), lotteryRes.data.prizeId && "99" !== lotteryRes.data.type && "3" !== lotteryRes.data.type && "8" !== lotteryRes.data.type && "9" !== lotteryRes.data.type && (message += `抽奖获得：${lotteryRes.data.name}\n`);
                else if (4001 === lotteryRes.code) {
        ...
        } else console.log(`目前热力值${total},不够抽奖`)
    }
}
...
function shareUrl() {
    return new Promise((resolve => {
        $.post(taskurl({
            apiMapping: "/active/shareUrl"
        }), (async (err, resp, data) => {
            try {
                5e3 === (data = JSON.parse(data)).code && console.log("尝试多次运行脚本即可获取好友邀请码"), 200 === data.code && (data.data && $.shareId.push(data.data), console.log(`\n【京东账号${$.index}（${$.nickName||$.UserName}）的${$.name}好友互助码】${data.data}\n`), console.log("此邀请码一天一变化，旧的不可用"))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
...
$.shareId = [], (async () => {
    if (cookiesArr[0]) {
        console.log("5G超级盲盒，可抽奖获得京豆，建议在凌晨0点时运行脚本，白天抽奖基本没有京豆，3小时运行一次收集热力值\n活动地址: https://blindbox5g.jd.com\n活动时间：2021-8-2到2021-10-29\n更新时间：2021-8-8 19:00"), $.http.get({
            url: "https://sub.timor.icu/JDscripts/jd_shareCodes.json"
        }).then((resp => {})).catch((e => console.log("刷新CDN异常", e))), await $.wait(1e3), await updateShareCodesCDN("https://sub.timor.icu/JDscripts/jd_shareCodes.json"), await $.wait(1e3);
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                ...
                await shareUrl(), await getCoin(), await Promise.all([task0()]), await taskList(), await getAward()
            } allMessage && ($.isNode() && await notify.sendNotify($.name, allMessage), $.msg($.name, "", allMessage, {
            "open-url": "https://blindbox5g.jd.com"
        })), $.shareId = [...$.shareId || [], ...$.updatePkActivityIdRes || []];
        for (let v = 0; v < cookiesArr.length; v++) {
            cookie = cookiesArr[v], $.index = v + 1, $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]), console.log("\n\n自己账号内部互助");
            for (let item of $.shareId) {
                console.log(`账号 ${$.index} ${$.UserName} 开始给 ${item}进行助力`);
                const res = await addShare(item);
                if (res && 2005 === res.code) {
