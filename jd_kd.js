...
function userSignIn() {
    return new Promise((resolve => {
        $.post(taskUrl(), ((err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(resp), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && (1 === (data = JSON.parse(data)).code ? (console.log(`今日签到成功，获得${data.content[0].title}`), message += `京东账号${$.index}${$.nickName}\n今日签到成功，获得${data.content[0].title} 🐶\n`, allMsg += message) : -1 === data.code ? console.log("今日已签到") : console.log(`异常：${JSON.stringify(data)}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve()
