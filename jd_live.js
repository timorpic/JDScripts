...
async function jdHealth() {
    $.bean = 0, await getTaskList(), await sign(), message += `领奖完成，共计获得 ${$.bean} 京豆\n`, await showMsg()
}

function getTs() {
    return (new Date).getTime() + 60 * (new Date).getTimezoneOffset() * 1e3 + 288e5
}
...
function getTaskList() {
    let body = {
        timestamp: (new Date).getTime() + 60 * (new Date).getTimezoneOffset() * 1e3 + 288e5
    };
    return new Promise((resolve => {
        $.get(taskUrl("liveChannelTaskListToM", body), (async (err, resp, data) => {
            try {
                if (err) console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`);
                    ...
                    if ((data = JSON.parse(data)).data.starLiveList)
                        for (let key of Object.keys(data.data.starLiveList)) {
                            ...
                            if (3 !== vo.state) {
                                ...
                                await superTask(vo.extra.liveId, authorId), await awardTask("starViewTask", vo.extra.liveId)
                            }
                        }
                    console.log("去做分享直播间任务"), await shareTask(), await awardTask(), console.log("去做浏览直播间任务"), await viewTask(), await awardTask("commonViewTask")
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
async function getauthorId(liveId) {
    let body = escape(JSON.stringify({
        ...
        url = `https://api.m.jd.com/client.action?functionId=liveDetailV910&build=167774&client=apple&clientVersion=10.1.0&uuid=${uuid}&${await getSign("liveDetailV910",decodeURIComponent(body),uuid)}`;
    return new Promise((resolve => {
        $.post(taskPostUrl("liveDetailV910", body, url), (async (err, resp, data) => {
            try {
...
async function superTask(liveId, authorId) {
    let functionId = "liveChannelReportDataV912",
        ...
        url = `https://api.m.jd.com/client.action?functionId=${functionId}&build=167774&client=apple&clientVersion=10.1.0&uuid=${uuid}&${sign}`;
    return new Promise((resolve => {
...
function awardTask(type = "shareTask", liveId = "2942545") {
    ...
    return new Promise((resolve => {
        $.post(taskUrl("getChannelTaskRewardToM", body), (async (err, resp, data) => {
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && ("0" === (data = JSON.parse(data)).subCode ? ($.bean += data.sum, console.log(`任务领奖成功，获得 ${data.sum} 京豆`), message += `任务领奖成功，获得 ${data.sum} 京豆\n`) : console.log(`任务领奖失败，${data.msg}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function sign() {
    return new Promise((resolve => {
        $.get(taskUrl("getChannelTaskRewardToM", {
            ...
            try {
                err ? (console.log(`${JSON.stringify(err)}`), console.log(`${$.name} API请求失败，请检查网路重试`)) : safeGet(data) && ("0" === (data = JSON.parse(data)).subCode ? ($.bean += data.sum, console.log(`签到领奖成功，获得 ${data.sum} 京豆`), message += `任务领奖成功，获得 ${data.sum} 京豆\n`) : console.log(`任务领奖失败，${data.msg}`))
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                resolve(data)
...
function taskPostUrl(function_id, body = {}, url = null) {
    return !url || "liveChannelReportDataV912" !== function_id && "liveDetailV910" !== function_id || (body = `body=${body}`), url || (url = `https://api.m.jd.com/client.action?functionId=${function_id}`), {
        url: url,
        body: body,
        headers: {
            Host: "api.m.jd.com",
...
function taskUrl(function_id, body = {}) {
    return {
        url: `${JD_API_HOST}?functionId=${function_id}&appid=h5-live&body=${escape(JSON.stringify(body))}&v=${(new Date).getTime()+60*(new Date).getTimezoneOffset()*1e3+288e5}&uuid=${uuid}`,
        headers: {
            Host: "api.m.jd.com",
...
function randomString(e) {
    ...
    16 == e && (t = "0123456789abcdefghijklmnopqrstuvwxyz"), e = e || 32;
    let a = t.length,
        n = "";
    ...
    if (cookiesArr[0]) {
        for (let i = 0; i < cookiesArr.length; i++)
            if (cookiesArr[i]) {
                ...
                uuid = randomString(40), await jdHealth()
            }
